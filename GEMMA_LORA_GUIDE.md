...
- 베이스: **Gemma 4 E4B**(`D:\models\teacherflow\gemma-4-E4B-it-*.gguf`, arch=`gemma4`). ⚠️ **"Gemma 3n"이 아님** — 2026-07-12 세대 혼동 실수로 3n에 학습했다 장착 단계에서 arch mismatch 발각(핸드오프 §25). **어댑터 작업 전 베이스 GGUF의 `general.architecture` 태그 확인 필수.** HF 가중치: `unsloth/gemma-4-E2B-it`(비게이트). 배포 타깃이 8GB 노트북이면 E2B 우선.
