...
>
> **최신**: §43 시리즈(라이브 트랙) 완결 — 표 생성 뒤죽박죽 해소(§43) → "오류" 고정 해소(§43b) → **프롬프트 다이어트 완료(§43d — 턴당 처리 토큰 -57% 실측, 2중 동봉 제거+프리픽스 캐시)**. 오프라인 160 PASS.
>
> **⏭️ 다음 1순위**: ①구형 실측 — E2B 축별 체감표(Notebook2 16일째 오프라인 → Notebook(100.89.219.102) 대체 후보, CPU/iGPU 프리필·생성 t/s + 축별 턴당 초) ②v15 데이터(견본행 교체 + 잉여값 부정 예제) ③보고서 본문 기능.
>
