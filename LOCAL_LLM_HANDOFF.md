...
- **하드벤치 베이스라인 1차(§16 태스크)**: 인벤토리 hwp 48·xlsx 37·hwpx 2. **xlsx 37/37 읽힘, hwpx 2/2 파싱**(표 3·6개, 빈칸 13·13 감지, **중첩 표 인식 확인**). hwp 48건은 COM 변환 필요(화면에 한/글 창 — 자율 시간 부적절) → 다음 단계 = hwp→hwpx 일괄 변환(사용자 재석 시) 후 전수 파싱률 + 빈칸 정답셋 만들어 gemma 채움 정확도 측정.
