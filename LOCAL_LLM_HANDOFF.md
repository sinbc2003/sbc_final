...
1. **표 인식·채우기 고도화** ("문서 제어를 inline AI보다 잘 되게 — 표 인식·칸 채우기 등등"):
   ...
   - **표갇힘형 셀 재구성**(§25 2차): kordoc이 기안서식 표를 뒤섞는 문제 — 셀 순서 복원하면 데이터 수율 +30%p 겸 표 파싱 견고화(일석이조).
   - §22 소품: E2B 편집 값변형·Excel 라이브채움.
2. **승인 UX** (inline AI '편집 전 확인하기'): live 액션 실행 전 diff 미리보기→승인/거절. `/api/chat/live/stream`이 액션을 pending으로 반환→프론트 카드→confirm API로 실행. 신뢰의 핵심.
3. 멀티파일/폴더 작업, 온보딩·모델팩 UI, 코드 서명(AV 대책).
