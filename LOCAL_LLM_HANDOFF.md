...
> ## ⏩ 다음 세션은 여기부터 (2026-08-16 갱신)
>
> **최신(8/17)**: §42p **v15 결과 = 채택 보류, v14 유지** — 정형 95%(+2)이나 자유문장 91%(-5). 원인 가설 = C 잉여값 예제의 정형 말투 대량 추가. §43d 다이어트(-57%)·§43e 구형 CPU 실측·`_is_placeholder` 데드코드 수정은 유효. 오프라인 163 PASS. **채움 채택 여전히 fill_g4e4b_v14.**
>
> **⏭️ 다음 1순위**: ①**v15 견본행 실물 비전 재검**(doc9류 — v15의 본 시험, 상단행 개선 확인 시 v16 = C 감량/다양화로 재학습) ②체인 인프라 수정(헬스 대기 연장·빈 출력=실패) ③보고서 본문 기능 ④GRPO·확신도 승인. 별건: Qwen3.8-27B Q4_K_M 수신 완료(`E:\llm\models\`, b10453 준비) — 구동 프로필 검증 대기.
>
...
>
> ### 현재 상태 (한 줄)
> **채움 채택 = fill_g4e4b_v14.gguf** (미학습 자유문장 **96%**·정형 93%, Opus -4%p) + 하네스 완비(§42 시리즈). **Desktop 개발 프로필은 v14 제품 배선 완료**(§42n — E4B Q4_K_M + 공문 v10(id0)/채움 v14(id1) + b10338, UI에 채움 어댑터 표시). 공문 생성 = E4B v10 / E2B v9(배포판). **배포 번들은 여전히 E2B v9**(번들 반영은 별도 결정).
>
> ### 다음 카드 (우선순위)
> 1. **v15 교육 데이터** — ①견본행 표 상단 1~4행 혼선(순번→날짜 복사)의 유일 잔여 처방: make_fill_dataset 확장, 표 상단에 견본값 주입 → 실값 교체를 gold로 ②**잉여값 부정 예제**: 양식에 없는 라벨의 값은 미채움이 gold(§42n 준비물 오배치 실측 — 지시문 값을 남는 빈칸에 밀어넣음).
...
> 4. 제품 통합(번들): 배포판 E4B+v14 전환 여부 결정, 번들 llama b10338 교체, 코드서명.
>
> ### 핵심 실측 궤적 (미학습 양식·엄정)
...
## 19. 작업 기록 — 2026-07-11 (E2B(2B) 모델 검증 ✅ — 8GB 노트북 배포 타깃 실측)

> §13 미검증 항목 해소. 모델: `gemma-4-E2B-it-Q4_K_M.gguf`(2.96GB, unsloth, hf-mirror 경유) — **위치: `D:\models\teacherflow\e2b\`** (⚠️ 상위 폴더에 두면 `_find_model` glob 알파벳순으로 엔진 기본 모델이 E2B가 돼버림 — 하위 폴더 격리로 E4B 기본 유지. E2B 테스트는 llama-server 수동 기동(:8400, 엔진이 실행 중 서버 재사용)).

### 결과 요약 — 핵심 사용례는 2B로도 완벽, 자유 편집만 열화
| 능력 | E2B 결과 | 비고 |
|---|---|---|
| 명단 파싱(스키마 강제) | **벤치 495/495 (100%)** | 역사 28명 포함 8표 전부 |
| 자율 배치(라벨그리드+enum) | 4/4, 본문+표 혼합 5/5 (무효ID 0) | E4B와 동급 |
| 생성(스캐폴드 프롬프트) | 구조 5/5, 1.6s/523자 | §13 "열화 예상"보다 좋음 — 구조를 코드가 잡아주는 덕 |
| 라이브 채움(채팅 "빈칸 채워줘") | 3/3 | |
| 질문 응답 | 정확 (직전 편집 반영) | |
| **자유 편집(블록 선택)** | **1/2 — 간헐 오선택** | 액션은 유효·실행 성공이나 엉뚱한 블록 선택 1회 관찰. **편집은 E4B 권장** |

- **발견+수정**: E2B가 값에 주변 문구를 중복 포함("김철수 (서명)") → 채움 프롬프트에 "빈칸 주변 문구((인)·(서명)·라벨)를 값에 반복하지 말 것" 규칙 추가 — 재실행에서 해소(E4B에도 이득).
...
**모델 × 능력 매트릭스** (`scripts/model_matrix.py`, RTX5080, provider=local·GGUF만 교체, 코드 수정 0):
| 모델 | 크기 | 벤치495(명단파싱) | 그리드채움(4빈칸) | tok/s |
|---|---|---|---|---|
| gemma-3n E4B | 4B | **495/495 (100%)** | 4/4 | 60~134 |
| gemma-3n E2B | 2B | **495/495 (100%)** | 4/4 | 106~231 |
| **Qwen2.5-3B** | 3B | **491/495 (99.2%)** | 4/4 | 260 |
| **EXAONE-3.5 (LG)** | 2.4B | **485/495 (98.0%)** | 4/4 | **303 (최속)** |
| Phi-3.5-mini | 3.8B | 77/495 (15.6%) | 0/4 | 227 |

**결론(일반화 경계)**: 하네스는 **한국어 능력 있는 소형 모델 패밀리 간에 일반화**된다 — gemma·Qwen·EXAONE 세 개의 완전히 다른 패밀리가 전부 제품 품질(98~100%), 코드 한 줄 안 바꿈. 단 **모델에 없는 한국어 능력을 하네스가 부여하진 못한다** — Phi(영어권)는 형식은 살렸으나 한국어 라벨↔값 의미매칭이 15.6%. 감사가 예측한 "형식=하네스, 값 의미=모델 한국어 능력"이 데이터로 확증됨. **EXAONE(한국어 특화)이 98%에 가장 빠름 — Qwen과 함께 배포 후보.**
...
### 실측 모델 위치 / 재현
- Qwen: `D:\models\qwen\qwen2.5-3b-instruct-q4_k_m.gguf`(2GB, hf-mirror Qwen 공식). Phi: `D:\models\phi\Phi-3.5-mini-instruct-Q4_K_M.gguf`(2.3GB, bartowski). EXAONE: `D:\models\exaone\EXAONE-3.5-2.4B-Instruct-Q4_K_M.gguf`(1.5GB, LGAI 공식). 전부 D:에만(리포·gitignore 밖). ⚠️ **C: 100%(1.5GB) — 모델·캐시는 반드시 D:**.
...
## 24. 작업 기록 — 2026-07-12 (LoRA 트랙 착수 — 데이터 소스 확정 + 데이터셋 파이프라인 v1 ✅)

> 가이드 §9 체크리스트 1·3번 진행. 사용자가 원본 문서 위치 확정 → 파이프라인 구축 → 부분 데이터로 전 단계 검증 완료.

### 데이터 소스 (사용자 확정, 총 4곳 — 원본 ~2,157 텍스트문서)
- **인수인계 폴더** `G:\내 드라이브\0.신병철의 Drive\0000. 2024학년도\2.교무부 업무\0. 인수인계`
  - 1. 기안문(본인 작성) 511 텍스트문서 — ★핵심. K-에듀파인 export: `[기관-공문번호 (본문|시행|첨부)] 제목.ext` 규칙 100% 일관(불일치 0), **(본문).odt = 공문 본문 그 자체**, 제목이 곧 지시문.
  - 2. 접수문 1,484 — 보조(선별). 3. 채용 9 — PII 밀도로 제외.
- **추가 소스(사용자 제공 2차)**: `...\17. 학교업무\01.2023 연구부\2023, 2024 이가영, 신병철 모든 공문`(123, 2023년도=연도 다양성) + `G:\내 드라이브\0. 경기과고\2. 교무\1. 2024년 모든 공문 및 파일`(39, **경기과학고=기관 다양성**). 파일명 규칙 동일 → 파이프라인 무수정 흡수.
- **로컬 복사**: `D:\lora_data\raw\{기안문,접수문,연구부공문,경기과고공문}` (odt/hwp/hwpx만 robocopy; G: 스트림 느림 — 기안문·연구부·경기과고 완료, 접수문 진행 중). ⚠️ 가정통신문 유형은 소스에 없음 — v1은 공문체만. 예상 최종 규모 **700~1,000쌍**(중복제거 후).
- **재학습 정책(사용자 질의로 확정)**: 데이터 추가 시 **전체 합쳐 처음부터 재학습**(버전제: dataset_v1→gongmun_v1.gguf, v2→v2...). incremental은 망각 리스크만 있고 RTX5080에서 재학습 수십 분이라 이득 없음. 어댑터 파일 교체 배포(merge 금지 §6.4)라 롤백 즉시.

### 파이프라인 = `scripts/lora/` 4단계 (데이터는 전부 리포 밖 D:\lora_data, PII라 깃 금지)
1. `inventory.py` — raw 순회→파일명 파싱→`manifest.jsonl`
2. `convert.py` — odt=pandoc(gfm), hwp/hwpx=기존 nodes 재사용. 멱등(캐시). **함정: pypandoc.convert_file은 경로를 glob 해석 → 파일명 `[...]`가 문자클래스로 깨짐 → pandoc.exe 직접 호출로 우회.**
3. `anonymize.py` — 정규식 PII 치환(주민번호·전화·이메일·계좌·생년월일·라벨기반 이름+지정실명 신병철→홍길동), `anon_report.json`에 통계·표본(**학습 전 육안 확인 필수**). 학습셋은 md_anon만.
4. `make_dataset.py` — kind∈{본문,시행}(본문 우선 대표 1건/공문번호), 필터(120자↓·8000자↑·표위주 70%↑), pandoc 이스케이프/blockquote 클린업, 제목 기반 지시문 템플릿 3종 로테이션 → `{"prompt","completion"}` train/val(95/5).

### 부분 검증 (복사 중 439건으로 전 단계 실행)
- 변환 **439/439 (100%)** (pandoc 309·hwp_node 124·hwpx_node 6) → 익명화 치환 535건(실명74·라벨이름305·전화121·이메일32·**주민번호2**·생일1) → 데이터셋 **183쌍**(train 174/val 9, 평균 370자). 표본 육안: 관련→본문→개조식(가나다)→붙임→"끝." 공문체 정확, 익명화 반영 확인.
- 전량 복사 완료 후 재실행하면 기안문만 ~300+쌍 예상(접수문 추가 시 확장).

### 다음 (가이드 §9 체크리스트 이어서)
- [ ] 복사 완료 → 전량 파이프라인 재실행 → `anon_report.json` 표본 육안 확인
- [ ] (2차) 지시문 하이엔드 역생성·llm_generate API 로깅, 참석자 명단 등 다중이름 익명화 보강(첨부 활용 시)
- [ ] unsloth로 E4B 어댑터 학습(RTX5080) → GGUF 변환 → `models/loras/` → **§5 배선**(--lora 프리로드 등 미완 구간)
- [ ] 검증: 벤치 495/495(어댑터 off) + 생성 품질 비교(on)

### 부수 (같은 날, 라이브 데모 세션)
- **멀티 문서/앱 자동인식 실증**: 프론트 문서 드롭다운+`/api/live/detect`(3초 폴링)+`/api/hwp/switch` 정상 — 한/글 3탭·Excel 워크북 목록/전환/"제어 중" 배지 확인. gemma 라이브 서식 명령("제목 굵게") 실행 성공.
- **한계 발견**: 탐색기 더블클릭은 별도 hwp.exe 프로세스가 될 수 있고 엔진은 ROT에 잡히는 인스턴스 하나만 봄 → 타 인스턴스 문서는 드롭다운에 안 보임. inline-ai급 위해선 다중 인스턴스 열거 필요(반나절급). pyhwpx `Hwp()`는 ROT `!HwpObject.` 모니커 우선 attach, 없으면 새 인스턴스 스폰(비결정적) — COM 작업은 엔진 경유가 안전.
- 이 과정에서 `_read_with_cvd` 캘리브레이션 "스캔 0 vs 블록 14" 경고 관찰(채움엔 무영향) — 셀 지목 서식 명령 시 재확인 필요.

---

## 25. 작업 기록 — 2026-07-12 (LoRA 학습 실행 — EXAONE 어댑터 완성·배선·실증 ✅ / gemma는 unsloth 전환)

### 🎯 헤드라인: 첫 공문 LoRA 어댑터 완성 + 엔진 배선 + A/B 실증
- **데이터셋 v1**: 940쌍(train 893/val 47, 평균 554자) — 접수문 파일명 변형 `[기관-번호 (유형) 발신처] 제목` 지원으로 262→940 확대.
- **EXAONE-3.5-2.4B 어댑터**: QLoRA(r16, 3에폭) **10분 50초 완주**(4.3s/step, VRAM 3~15GB 스필 없음), eval_loss 1.283(ep1)→**1.172**(ep3). → `convert_lora_to_gguf.py` → **44MB gguf** (`D:\models\loras\gongmun_exaone_v1.gguf`, 리포 models/loras에도 사본·gitignore).
- **A/B 실증(엔진 경로 `llm_manager.generate(lora=)`)**: OFF=마크다운투 안내문 / ON=**정확한 공문체**("1. 관련: … 2. …안내하오니 협조하여 주시기 바랍니다. 가나다라 개조식, K-에듀파인 언급"). 오프라인 44검사 PASS.
- **배선(§5 완결, `7579c8c`)**: 서버 기동 시 `--lora-scaled <어댑터>:0.0` 프리로드(기본 OFF=추출·분류·벤치 무영향) + `_local_chat_completion` per-request `lora:[{id,scale}]` + `_ensure_local_server`가 `/lora-adapters` 프로브(재사용 서버 포함) + `_find_lora` 탐색을 `loras_dirs`설정→`D:\models\loras`(ASCII)→리포 순으로 확장. llm_generate lora [WARN] 제거.

### ⚠️ 실측 함정 카탈로그 (전부 스크립트/엔진에 주석 박제)
1. **pip이 torch를 CPU 빌드로 교체**(timm→torchvision 연쇄) → cu128 인덱스로 재고정. venv 설치 후 `torch.cuda.is_available()` 확인 필수.
2. **C: TEMP 고갈**: pip 압축해제가 TEMP 사용 — TMPDIR/TMP/TEMP를 D:로. (ESTsoft가 tempdir 하이재킹하는 프로세스도 있음.)
3. **gemma-3n AltUp × 4bit**: 학습 중 `prediction_coefs.clamp_()`가 uint8에 실패 → `llm_int8_skip_modules`. **tf5 매칭은 접두사/regex(re.match)/접미사만** — 경로 중간 토큰은 `".*altup.*"` 형태.
4. **tf5 `torch_dtype` 무시** → 비양자화 파트 FP32 → 공유메모리 27.6GB 스필, 400s/step. `dtype=` 사용(tf4는 반대로 torch_dtype만).
5. **EXAONE 3.5 원격코드는 현재 tf5 타깃**(RopeParameters 임포트 — tf4 venv 불가). 단 포팅 불완전: ①`get_input_embeddings` 미구현→wte 인스턴스 패치 ②`create_causal_mask`에 구인자(input_embeds/cache_position) 전달→시그니처 정합 심(스크립트 내). tf5 신뢰 프롬프트는 Windows SIGALRM 크래시 — **tokenizer 포함 모든 from_pretrained에 trust_remote_code=True**.
6. **llama-server ANSI argv**: 한글 경로 어댑터 로드 실패(mojibake) → 어댑터는 ASCII 경로. **`--lora-scaled`는 `FNAME:SCALE` 콜론 결합**인데 드라이브 콜론(D:)과 충돌 → **어댑터 디렉터리를 cwd로 상대경로** 전달(엔진도 동일 처리 필요 시 참고 — 현재 엔진은 절대경로 리스트 인자라 재검증 필요⚠).
7. **gemma-3n은 바닐라 HF QLoRA로 16GB 불가**: E4B 27.6→12GB(수정 후에도) 스필 400s/step, **E2B도 13GB 스필 273s/step** — 원인은 양자화 불가(Embedding) 레이어들(어휘 26만 embed_tokens + per-layer embeds ≈6GB) + 타워. **해법 = unsloth**(gemma-3n 전용 최적화, venv_unsloth 구축 중) 또는 야간 바닐라(12~18h).

### 환경/자산
- 학습 venv: `D:\lora_train\venv`(tf 5.13/peft 0.19/bnb 0.49, torch 2.11 cu128), `venv_tf4`(4.51.3 — EXAONE엔 불필요해짐), `venv_unsloth`(구축 중). HF 가중치: `D:\models\hf\{gemma-3n-E4B-it,gemma-3n-E2B-it,EXAONE-3.5-2.4B-Instruct}`. llama.cpp 리포: `D:\lora_train\llama.cpp`(변환 스크립트+gguf-py editable).
- 학습 실행은 **detached**(Start-Process)라 클로드 코드 종료와 무관. 로그 `D:\lora_train\*.log/.err`.
- 산출: `D:\lora_train\out\gongmun_exaone_v1\final`(HF 어댑터) + gguf. train_loss 1.228.

### gemma-3n 학습 경로 실측 (함정7 후속 — unsloth도 막힘, bf16이 답)
- **unsloth 막다른 길 2개**: ①로컬 fp16 로드 시 자기들 4bit화도 AltUp 계수를 양자화 → 같은 clamp 크래시(transformers 소스+컴파일캐시에 float-가드 패치로 통과 가능) ②사전양자화 체크포인트(unsloth-bnb-4bit)는 tf 5.5 로딩에서 일부 레이어 quant_state 미부착 → unsloth_zoo 패치의 무양자화 폴백이 packed uint8을 F.linear에 투입 → 셰이프 크래시. unsloth 2026.7.2+tf 5.5.0+bnb 0.49.2 조합 버그(Windows).
- **최종 경로 = `--no-4bit`(bf16 그대로, batch1)**: 양자화 함정 원천 소멸(AltUp clamp는 float라 정상). E2B(5.4B raw) bf16도 13GB 스필은 나지만 **48~50s/step = QLoRA 스필(273s) 대비 8배** → 168스텝 ≈ 2h20m으로 당일 완주 가능. AltUp float-가드 패치는 venv_unsloth의 transformers에만 적용(주 venv는 무관).
- E4B는 bf16 15.7GB 웨이트라 스필 확정 — 야간 코스(bf16이 QLoRA보다 빠를 것, 실측 후 결정).

### ⚠️⚠️ 중대 실수 기록 — 베이스 세대 불일치 (2026-07-12 오후 발견)
**gemma 어댑터를 하루 종일 엉뚱한 세대에 학습했다.** 배포 GGUF(`D:\models\teacherflow\gemma-4-E2B/E4B-it-*.gguf`)의 arch 태그 = **`gemma4`(Gemma 4, 2026 세대)**인데, HF에서 이름 유사성으로 **`gemma-3n`-E2B/E4B(전 세대)**를 받아 학습함 — 3n E2B 어댑터 완주(train_loss 0.624) 후 장착 시점에 "model arch and LoRA arch mismatch"로 발각. **교훈: 어댑터 작업 전 반드시 베이스 GGUF의 `general.architecture` 태그부터 확인**(`gguf.GGUFReader` 몇 줄). 파일명·핸드오프 서술("Gemma 3n E4B(현재 gemma-4-...gguf)")도 이 혼동을 부추겼음 — 본 문서 §6.2 표기 정정.
- **부수 소득**: 오늘의 함정 카탈로그(AltUp·스필·unsloth)는 전부 gemma-3n 특유 구조의 문제였고, **Gemma 4는 표준(Gemma3 계열) 구조**라 llama.cpp `Gemma4Model(Gemma3Model)` — tf 5.13 네이티브 지원, HF `unsloth/gemma-4-E2B-it`(10.25GB, 비게이트).
- **Gemma 4 E2B 실측**: QLoRA(batch2)도 10.2GB 스필·193s/step → **bf16(--no-4bit)+batch1로 전환**(3n E2B를 완주시킨 검증 경로, 그때 실측 6~9s/step). 3n 가중치·산출물은 폐기(공간 회수), 3n 어댑터 final만 기록용 보존.
- **무음사(無音死) 노하우**: bf16 스필 학습이 트레이스백 없이 죽는 사례 2회(에폭 평가 시점 1회·저장 도중 1회) — ①`--no-eval`(평가 배치 스파이크 제거) ②`save_steps=28` 중간 저장 ③`--resume` 이어달리기 ④프로세스 생사 워치독(2분 주기)으로 대응 체계 완성. 저장 중 죽으면 해당 checkpoint 디렉터리가 README만 있는 껍데기가 됨 — 재개 전 완전성 확인(adapter_model.safetensors 존재).

### ✅ gemma-4 E2B 어댑터 완성 (2026-07-12 13:35)
- bf16 batch1 학습 **53분**(19s/step, 무음사 없음), train_loss 1.2 → `gongmun_g4e2b_v1.gguf`(48MB, `D:\models\loras\`).
- **배포 GGUF(`gemma-4-E2B-it-Q4_K_M`) 장착 A/B 실증**: OFF=마크다운 문서투(문서번호/시행일자 템플릿) / ON=**기안문**("1. 관련: … 2. …수립하여 운영하고자 합니다. 붙임 … 1부. 끝."). arch 일치 확인.
- **최종 산출물 2종**: EXAONE(`gongmun_exaone_v1.gguf` 44MB) + gemma-4 E2B(48MB) — 각 베이스에서 ON/OFF per-request 토글 검증 완료.
- 개념서 PDF 제작(Edge headless) → **coka 텔레그램 전송 완료**(cokacdir_desktopbot, msg 1077).

### 전국 공문 시범 트랙 (2026-07-12 오후 — 사용자 크롤러 자산 활용) ✅
- **발견**: Mac1 `~/shared/document_craw`(매일 06시 cron) = open.go.kr 전국 교육청 공문 **359만 건 메타데이터**(제목·기관·부서·문서번호) + 첨부 다운로드 파라미터. **본문은 미수집**(body_text 0% — 본문이 '결재문서본문.pdf' 첨부에 있음). PROJECT.md에 "LLM 학습 데이터" 목적 명시돼 있었음.
- **다운로드 체인 재현**(`pilot_download.py`, Mac1 배치): wonmunFileRequest→Response 폴링→Download 3단계. 함정: ①응답이 modelAndView.model.result 중첩 ②원기관 시스템 다운 = 영구실패(2022년산 다수 → **2024+ 문서로 한정 시 성공률 90%**) ③크롤 필드 타입 불균일(list 섞임). 교육청별 상한 60으로 지역 다양성 확보. **결과: 970 PDF**(문서당 2초 간격, ~50분).
- **변환**: kordoc vs PyMuPDF 20건 실측 — kordoc 승(996자 vs 661자, 표 보존 vs 상실). `pilot_convert.py` → **985/985 성공**.
- **본문 추출**(`pilot_extract.py`): 레이아웃 2유형 — 산문형(제목라인→본문→"끝.") 53% 추출 vs 표갇힘형(kordoc이 기안서식 표 셀을 뒤섞음) 30% 스킵. 제목 여러 줄 감김 꼬리 제거 처리. **523쌍**(중앙값 429자, 이상 0.6%), 익명화 적용. **프롬프트 절반에 "기관: ○○교육청" 컨텍스트 주입** — 기관 지정 생성을 어댑터에 내재화(프롬프트 제어 실측 근거). → `D:\lora_data\dataset\gongmun_national.jsonl`
- **스케일 전망**: 시범 1,000건 → 523쌍이므로 풀 코퍼스(350만)에서 이론상 수십만 쌍. 표갇힘형 복원(셀 순서 재구성)하면 수율 +30%p 여지.

### 전국 523쌍 품질 검증 완료 (2026-07-12 밤 — 사용자 지시 "검증부터")
- 표본 30 + 전수 스캔: ①"끝. 미종결" 110건 → **끝. 절단+말미 태그 제거 후처리로 0건** ②휴대번호/이메일 "잔존"은 전부 **우리 더미값 오탐**(실 PII 0건) ③본문 내 HTML 표는 뒤섞임 없이 온전(예산표 rowspan 정상) — v1 코퍼스와 형식 일관.
- **판정: v3 투입 가능.** 523쌍, 중앙값 413자, 이상 시작 0.6%.

### 남은 것 (다음 세션 인수인계)
- [ ] gemma-4 E4B — **사용자가 67%(112/168)에서 학습 취소**(2026-07-13 00:35경). checkpoint-84 보존 → 재개 시:
  `D:\lora_train\venv\Scripts\python.exe scripts\lora\train_lora.py --model D:\models\hf\gemma-4-E4B-it --out D:\lora_train\out\gongmun_g4e4b_v1 --max-len 2048 --batch 1 --grad-accum 16 --no-4bit --no-eval --resume D:\lora_train\out\gongmun_g4e4b_v1\checkpoint-84` (잔여 84스텝 ≈ 5.9h, TMP/HF_HOME은 D:로).
  완주 시: `convert_lora_to_gguf.py final --base D:\models\hf\gemma-4-E4B-it` → `D:\models\loras\gongmun_g4e4b_v1.gguf` → 배포 GGUF(`D:\models\teacherflow\gemma-4-E4B-it-Q3_K_S.gguf`) 장착 A/B. (E2B 어댑터가 이미 배포 타깃을 커버하므로 E4B는 선택 사항.)
- [ ] **v3 재학습**(v2 1,034 + 전국 523 ≈ 1,557쌍, 검증 완료) — GPU 확보 시. make_dataset에 gongmun_national.jsonl 병합 로직 추가 필요(현재 별도 파일). 학습 후 유형 혼선 검사(공문 5·계획서 5 생성) → 혼선 시 어댑터 분리.
- [ ] (2차) 표갇힘형 셀 재구성(+30%p 수율), 장문 계획서 청킹, 전국 다운로드 스케일업(350만 풀), 관련번호 후처리 치환·학교명 설정 주입 제품화, E2B 편집 정확도, repeat penalty.
- [ ] 프롬프트 제어 실측(EXAONE 어댑터): **기관명 지정은 프롬프트로 완벽 동작**(가상 학교명 일관 적용, 미지정 시 수원외고 편향). **관련(근거번호) 억제는 프롬프트로 불가**(스타일 각인) → 후처리 치환 필수 + v2 데이터에서 placeholder화. 간헐 반복 루프 → repeat penalty 검토.
- [ ] 엔진 `--lora-scaled` 인자 형식 실검증(드라이브 콜론 이슈 — cwd 상대경로 방식으로 수정 필요 가능성 높음⚠).
- [ ] settings.llm에 `local_lora`/`loras_dirs` 문서화 + 프론트 노출(디자인 §6.4 "문서 스타일" 버튼).
- [ ] 검증 안전망: 어댑터 off 벤치 495/495 회귀(§8) — 아직 미실행(서버 재기동 필요).
- [ ] 개념서 PDF → coka 텔레그램(사용자 요청, 작업 종료 후).
- 관련 번호 환각(관련: 기관-번호) — 생성물 후처리/프롬프트로 실제 관련 근거 주입 검토(2차).

---

## 26. 작업 기록 — 2026-07-13 (TeacherFlow 개선 트랙 — 설계부 등, 사용자 지시)

> 지시: "teacherflow 설계부분 등 개선점 파악후 개선 시작. md최신화해서 세션 만료에도 이어서."
> 상태: **4렌즈 병렬 분석 완료 → 개선 백로그 확정(아래) → 구현 진행 중.** 이 섹션이 진행 원장이다.

### 사전 완료 맥락 (같은 날 새벽)
- TeacherFlow LoRA 통합 완성(`e8ebee5`): 설정 E2B+gongmun_g4e2b_v1, llm_generate 설정 폴백, DRY 샘플러(repeat_penalty는 주제이탈 실측), 제품 /api/run E2E 2회 검증.
- 폴더 정리(`b58c4e3`): 루트 잡동사니 → _archive/·docs/ (참조 grep+테스트 PASS).

### 개선 백로그 (4렌즈 병렬 분석 31건, 전문 `D:\lora_data\tf_improvement_findings.json`)
우선순위 = impact/effort. **[✅]=완료, [ ]=대기.** 상위(H/S)부터 구현.

**H/S (고효과·저비용 — 최우선) — 12건 전부 완료 ✅**
- [✅] 채팅 기본모델 OpenAI 하드코딩 → **429 유발**(실사용 보고). active 모델 선택(`6b8392a`).
- [✅] TaskRunner가 엔진 한국어 에러 버림 → done.errors 그대로 노출(`2157d9b`)
- [✅] 실패 노드 영구 스피너 → done 시 잔여 running을 error로 마감(`2157d9b`)
- [✅] run-stream done에 output_files 추가 + 자동열기(`2157d9b`)
- [✅] `/api/run` 이벤트루프 블로킹 → asyncio.to_thread(`2157d9b`)
- [✅] 생성 워크플로우 저장 전 검증 → validate_workflow(노드type/엣지/포트, 사유안내)(`621b757`)
- [✅] Ctrl+S stale closure 중복생성 → getState() 최신조회+설계모드 한정(`7204e62`)
- [✅] 미저장 유실 → 새 워크플로우 confirm + beforeunload(`7204e62`)
- [✅] 파라미터 description UI 노출 + 숫자 NaN→기본값 폴백(`7204e62`)
- [✅] **공문 LoRA 원클릭 프리셋** `preset_gongmun_gen`(제목→공문→hwpx, 홈카드·UI 검증)(`ddef444`)
- [✅] FormAssist 로컬 LoRA provider 노출(API키 없어도 선택 가능)(`184a679`)
- [✅] 워크플로우 → 홈카드 등록 UI(저장메뉴, /preset route E2E 200)(`184a679`)

**H/M**: 실행 중 취소 수단 전무(`runner.py:239`) · **워크플로우 생성 GBNF 강제 부재**(`chat/workflow.py:236`, live_chat 패턴 복제) · Ctrl+Z 부재(zundo) · 연결실패 무음(`FlowCanvas.tsx:61`) · 저장 워크플로우 카드 실행버튼 부재 · 고빈도업무(가통문·계획서·명단) 프리셋 전무

**M/S**: 스트림 1000자 무음절단 · TaskRunner node_progress 무시 · **_auto_layout 엣지키 버그(source/target vs from/to — 자동배치 죽음, 확정)** · from_port 누락 KeyError→500 · JSON파싱실패 원문노출 · 채팅 모델 openai 하드코딩폴백 · 우클릭 노드추가 좌표 미변환 · 입력포트 다중연결 무음덮어쓰기 · TaskRunner 입력라벨 메타 미사용

**M/M**: 실행이력 재사용불가+node_timings 형식깨짐 · FormAssist 채팅로직 2곳 중복 · defaultNodes 드리프트(lora 누락, yaml 생성기) · llm_generate LoRA 자유텍스트(어댑터 목록 API) — **L/S**: 채팅 history 무제한 누적

### 구현 기록 (H/S 9건 완료, 2026-07-13)
- `6b8392a` 채팅 기본모델 = active 모델(429 수정) · `2157d9b` 실행 피드백 4종(블로킹/에러/스피너/출력파일) · `621b757` 워크플로우 생성 검증 + auto_layout 버그(source/target→from/to) · `ddef444` 공문 초안 작성기 프리셋 + TaskRunner 라벨 메타 · `7204e62` 설계캔버스 3종(Ctrl+S/미저장/파라미터) · `184a679` FormAssist 로컬 LoRA + 홈카드 등록 UI
- **검증**: offline 테스트 PASS · validate/auto_layout 단위테스트 · 프리셋 E2E(hwpx 출력) · /preset 등록 route E2E 200 · 브라우저 홈카드·입력화면 확인 · tsc 클린
- **세션 종료(2026-07-13 새벽): H/S 12/12 완료.** 다음 세션 이어서: **H/M**(실행취소·**워크플로우 생성 GBNF 강제**(live_chat 패턴 복제, chat/workflow.py:236)·Ctrl+Z(zundo)·연결실패 피드백·저장카드 실행버튼·고빈도프리셋 가통문/계획서/명단) → **M/S**(스트림절단·node_progress·from_port KeyError·우클릭좌표·다중연결·JSON파싱실패) → M/M·L/S. 전문 `D:\lora_data\tf_improvement_findings.json`.
- **환경 상태(다음 세션 재개용)**: 엔진 :8407(설정 E2B+gongmun_g4e2b_v1, adapter scale0 프리로드) + Vite :1420 기동 상태. 어댑터 2종(`D:\models\loras\gongmun_{exaone,g4e2b}_v1.gguf`). v3 데이터셋(1,557쌍) 대기 — 재학습 시 GPU 필요. E4B checkpoint-84 재개 가능(§25).
...
## 30. 작업 기록 — 2026-08-08 새벽 (프롬프트 vs LoRA 실측 → 2단 프리셋 + v5 역생성)

> 사용자 질문("프롬프트만 vs LoRA 결합?")에 실측으로 답한 트랙. 결론 = **역할 분담**: 서식=LoRA(프롬프트로는 소형모델에 기안문 서식 못 새김 — 실측), 사실=프롬프트+데이터 설계, 정확성=코드.

**핵심 실측(v4에서 발견)**: 같은 사실통제 프롬프트로 A/B → LoRA ON은 서식↑·사실 왜곡(기간 틀림·환각), OFF는 사실 100%·서식 마크다운투. **원인 = 학습 instruction이 '제목→창작'이라 개요 준수를 배운 적 없음**(make_dataset 주석의 "역생성은 2차 레버" 바로 그것).

**Track A — 공문 정밀 작성기(2단) 프리셋 ✅**(`preset_gongmun_2stage`, `de74f91`): 1단 베이스(lora none, 사실통제 규칙+{{오늘한글}}) 내용 확정 → 2단 LoRA(서식 변환, 사실 불변 지시, temp 0.2) → hwpx. E2E: 핵심 사실 전부 정확+기안문 서식+placeholder 후처리. 텔레그램 msg 1120.

**Track B — v5 역생성 ✅**: `scripts/lora/abstractify.py`(베이스 gemma+JSON 스키마로 완성문→개요 역추출, 멱등·2.3s/건) → 1,141/1,374 성공(83%) → make_dataset v5: **개요 instruction 70% 혼합**(3종 템플릿, 제목 30%·축약증강 유지) → 학습 134분(loss 0.934) → `gongmun_g4e2b_v5.gguf`.
**v5 검증**: ①사실 순응 — v4에서 왜곡되던 기간·마감 **전부 정확**(교과·인원·기간·전형·마감 100%) ②단일 프롬프트 567자 상세(무맥락 창작 환각은 여전 — 개요 없으면 창작 모드, 프리셋 경로 무관) ③계획서 혼선 소폭 복귀(공문서식+HTML 표 잔재 유입) — **제품 경로에서 어댑터는 공문 프리셋에만 적용되므로 실노출 없음**(v4 감사에서 봉합 확인) ④수원 편향 0 유지 ⑤벤치 495/495. **판정: v5 채택**(사실 순응이 제품 핵심), 번들·포터블 v5 반영 완료.
**v6 데이터 백로그**: HTML 표 잔재 필터 강화 · 무맥락 창작 억제(빈 개요→"개요를 요청하라" 학습?) · Mac1 신규분 합류(아래) · 관련 라인 중복 정리.

**v5 스케일업 진행**: Mac1 6,000건 배치 가동 중(새벽 3,098+ PDF, 성공률 90%). **완료 후 체인**: rsync→pilot_convert→pilot_extract→abstractify(신규분)→make_dataset→v6 학습. pilot_download에 --per-org-cap 인자화(`5697171`).

### v6 체인 (2026-08-08 오전 — 진행 원장)
- Mac1 배치 종결 **4,680 PDF**(후보 소진) → tar 스트림 전송(10분) → pilot_convert **4,679/4,679**(kordoc 3,672·pymupdf 22) → pilot_extract **2,504쌍**(수율 53.5%, 표갇힘 스킵 1,375·실패 785) → abstractify 신규 +1,590(누계 2,755) → **v6 데이터셋 3,781쌍**(train 3,592/val 189, 개요 instruction 1,984쌍).
- **v6 학습 진행 중**(pid 34872, 651스텝): SBC-LLM-Agent 워커(재부팅 후 재기동)와 VRAM 경합으로 50~63s/step — ETA 오후 5~6시. 사용자 "너가 알아서" + UAC 미승인 → 워커 공존 수용, 간섭 없이 완주 대기. 완료 후: GGUF→검증 배터리(사실 순응 A/B·단일 프롬프트·혼선·벤치)→번들 v6→보고.

### v6 완료 ✅ (2026-08-08 오전) — 데이터 2배판 채택
- **학습**: 651스텝. 워커 VRAM 경합으로 50~63s/step 저속 → **교훈: 스필된 학습은 VRAM이 풀려도 자동 회복 안 됨 — checkpoint 재시작이 정답**(280에서 재개 후 6.5s/step 3배 회복, 71분 완주). loss(재개 구간) 0.368.
- **검증**: 사실 필드 정확 유지 · **관련 라인 과잉 6→3** · **계획서 혼선 소멸**(관련 0, 개조식 계획서) · 수원 0 · 벤치 495/495 · "  끝." 후처리 ✓. 신규 노출: **파생 산수 환각**("총4개월" — 9.1~2.28은 6개월) → 2단 프리셋 규칙에 "파생 계산 금지" 추가. 간결 요청 시 연도 가정은 제품 경로({{오늘}} 주입) 무관.
- **결문 표기 규정(사용자 지적)**: 변환기 공백 정규화로 데이터 99%가 "1부. 끝."(한 칸, 2,873:22 실측) → ①llm_generate 후처리 강제("  끝.") ②make_dataset 정규화(v7부터 모델이 학습). `b00aa4f`.
- **비교 산출물(텔레그램)**: 보고 1121 · 프롬프트만 vs LoRA 1122/1123 · 비교 PDF(상세+간결 4종) 1124 · 프롬프트 원문 1125 · **v5vs v6 비교 PDF 1126**. PDF 제작 = HTML→Edge headless(--print-to-pdf) 패턴.
- **번들 v6 반영**(stage_bundle·NSIS·포터블). 어댑터 계보: v1(940)→v3(1,557 공문특화)→v4(placeholder)→v5(역생성)→**v6(3,781쌍, 배포 채택)**.

### v7 미채택 → v8 채택 ✅ (2026-08-08 오후 — 자율 사이클 2회)
- **v7**(관련 1줄 축약 543쌍): 관련 1줄 달성했으나 **HTML 표 잔재 유출·결문 누락 회귀** → 미채택. 원인 = 데이터 28%가 kordoc HTML 표 포함(학습 타깃 오염).
- **v8**(관련 축약 + HTML표 873쌍 드롭, 2,803쌍): **개요 경로 역대 최고** — 관련 1줄·전 필드 정확·"1부.  끝." 자체 학습·HTML 유출 0. **2단 프리셋+v8 최종 산출 = 관련 1줄·사실 100%·환각 0·규정 결문**(PDF 텔레그램 1127). 벤치 495/495. **채택·번들 반영**.
- **한계 기록**: v8 어댑터 단독 간결 한-줄 요청은 서식 각인 콘텐츠 유출(특목고 동의서) — 제품 경로 미노출(채팅=베이스·프리셋=개요)이나 **v9 과제 = 간결 요청 견고화**(간결→개요 확장을 데이터로? or 채팅 공문 의도 감지 시 2단 파이프 라우팅이 정공법).
- **계보**: v1→v3(공문특화)→v4(placeholder)→v5(역생성)→v6(2배 일반화)→v7(폐기)→**v8(정화판, 배포 채택)**.

### §31 저녁 진행 (2026-08-08) — 규정·실험·재료
- **공문서 작성법 매뉴얼 반영 ✅**: 경기도교육청 「한 곳에 정리한 공문서 작성법」(웹 확보, 행정업무규정 제7조 근거) → 표기 정규화 후처리+데이터 이중 반영: 날짜 'YYYY. M. D.'(온점 뒤 1타·0 미표기)·관련 '호' 삭제·붙임 뒤 2타·연속행 1. 정렬(전각2+2타)·**첨부 단수 무번호**·(학교명) 폴백·문장병합·들여쓰기(하위 2타)·"  끝.". PDF 텔레그램 1129. (`5fa3ca0`)
- **few-shot vs LoRA 실측(사용자 질문 "LoRA 무용론?") ✅**: base+예시2 시스템 프롬프트 vs v8 동조건 — **상세 개요 경로는 동급**(무용론 부분 인정), 차이는 ①간결·변칙 견고성(few-shot은 예시 라벨 복사·연도 이탈) ②프롬프트 토큰 4~10배(저사양 프리필·컨텍스트 잠식) ③일관성. 결론 = provider별 분담: **로컬=LoRA, API=few-shot** → `engine/skills/gongmun_fewshot.md` + llm_manager.generate에 lora=gongmun→API 시 자동 주입 편입(`0cf73d4`).
- **양식 첨부 수집 실측**: 첨부 서식류는 **대부분 비공개**(628/720 "비공개 파일") — 구조적 한계. **87건 확보**(`D:\lora_data\form_bench` — hwp/xlsx 실전 양식). 하드벤치 시드로 시작, 스케일은 공개 서식 자료실(교육청 홈페이지) 크롤 등 대체 소스 검토.
- **하드벤치 베이스라인 완성(§16 태스크, "go" 승인)**: hwp 48건 COM 일괄 변환 **48/48**(§19 처방: Hwp.exe 선실행 후 COM — CO_E_SERVER_EXEC_FAILURE 재현·해소, 웜 상태 9초). 전수 파싱 **50/50 성공, 표 380개(중첩 53), 빈칸 1,491개 감지**(xlsx 37/37 별도). 통계=`D:\lora_data\form_bench\parse_stats.json`.
- **핵심 발견(하드벤치의 존재 이유)**: **빈칸 0건 문서 20/50** — 실전 양식 상당수가 우리 빈칸 감지(라벨된 표 빈칸·누름틀·본문 밑줄) 밖의 형태. E2E 실증: 도시숲 참가신청서(표 2·감지 빈칸 0) → 텍스트 치환 폴백로 흘러 **gemma 의미 매핑은 30항목 정확했지만 주입은 3필드뿐**. → **§32 후보 1순위 = 빈칸 감지 커버리지 확장**(감지 0건 20종 유형 분석: 셀 병합형 입력칸·괄호형 ( )·콜론 뒤 공백형 등) — 감지만 넓히면 배치 능력은 이미 검증돼 있음.

### §32 ① 감지 병리 수정 ✅ (2026-08-08 밤, `86713d5`)
- **유형 분석(감지 0건 20종)**: 절반이 "빈 셀은 있는데 감지 안 됨" — 원인 추적 결과 감지 로직이 아니라 **`header_row_count()` 병리**: 라벨-값 양식(신청서류, 행마다 병합)에서 "상단 연속 병합" 휴리스틱이 **전 행을 헤더로 판정**(11/11행·19/19행 실측) → 모든 빈칸이 "헤더 구간" 스킵.
- **수정**: 5행 이상 표에서 전행=헤더 판정 시 1로 폴백(그리드형은 헤더가 전 행일 수 없음 — 소형 표(≤4행) 요약 상속 동작 보존).
- **효과**: 실전 50종 감지 **1,491→3,495(+134%)**, 감지 0건 **20→9**(잔여 = 선서문·안내문 등 실제 채울 칸 없는 서술형 다수), 라벨 품질 정확("담당자명" 등 row_header 활성). **벤치 3종 무손상**(재인식 495/495·왕복 1126/1126·레벨2 495/495). E2E: 참가신청서 폴백→**그리드 경로 배치·검증 2/2**.
- **잔여(§32 계속)**: 감지 0건 9종 재분류(진짜 서술형 vs 놓친 유형) · 괄호형 ( )·콜론형 입력칸 감지 · xlsx 채움 하드벤치 · hwp 변환 자동화(COM 배치 스크립트화).

### §32 ② 부분 슬롯 감지 — 괄호형·콜론말미형 ✅ (2026-08-08)
- **0건 9종 재분류 결론**: 4종=이미 작성 완료된 문서(차량신청서에 소나타·차대번호, 권한신청서에 실명 기재 — **감지 0이 정답**), 1종=안내문(표 없음), 4종=견본형 양식(홍길동·○○학교·'2026년 월 일' 견본문자 — 빈칸 채움이 아니라 "견본 교체" 작업, 편집 경로 소관). → 0건 자체는 문제가 아니었고, 진짜 갭은 **내용 있는 셀 안의 부분 입력칸**.
- **48종 센서스(정량)**: 괄호형 `( )명` **67셀/11문서**, 콜론말미형 `소 속 :` 8셀/5문서, 견본혼합형 `○○과` 143셀/17문서(오탐 위험 커서 보류).
- **구현**(hwpx_grid.py + form_assist.py):
  - `_cell_slot_type()`: 빈 괄호쌍 → "paren", 셀 전체가 짧은 라벨+콜론 → "colon". 콜론은 우측 이웃 가드 — 빈 이웃(그쪽이 슬롯)/값·견본 이웃(이 셀은 라벨)이면 제외, 이웃도 콜론말미('1순위:|2순위:')면 연쇄 슬롯 인정.
  - `extract_blank_fields`: 슬롯 셀은 include_filled 무관 포함, value_type "paren"/"colon", 라벨에 "괄호 안/콜론 뒤에 들어갈 값" 명시(프롬프트의 "값 있는 칸은 그대로" 지시와 충돌 방지).
  - `_compose_slot_value()` (fill 경로): 원문 보존 삽입 — 콜론은 뒤에 이어쓰기, 괄호는 첫 빈 쌍 안 삽입(원 괄호문자 유지), **빈 값으로 라벨 클리어 금지**. 일반 셀은 기존 의미(교체/""=클리어) 불변.
  - form_assist: `_is_fillable`·`_plan_grid_fill`·양쪽 추출부가 paren/colon 수용.
...
### §35 v9 채움/편집 LoRA — 데이터 트랙 개시 🔵 (2026-08-09 새벽, `8f62cce`)
- **사용자 결정(8/9)**: v9 = 공문 서식이 아니라 **편집 액션 선택(block_id 오선택 축) + 채움 값 형식**을 겨냥. 크롤의 2026 양식 첨부로 연습 승인. (배치 정확도 자체는 코드 구조가 이미 포화 — 셀ID는 GBNF enum이라 모델이 좌표를 안 다룸.)
- **크롤 실측 (Mac1 345만 문서)**: 양식성 첨부 총 18.2만 / 파일명 '2026' 포함 1.5만 (생산연도 2026은 백필 미도달로 61건 — "2026년 기준"의 실체는 2026학년도용 양식). **핵심 발견: 기작성 문서(명단·현황 수합본)는 '비공개 파일'로 다운로드 불가(1차 배치 87% 영구실패)** → 실채움 문서 수확 전략은 이 소스에서 규모 불가.
- **전략 피벗 — 값 합성이 주 경로**: 공개인 빈 서식 + 라벨 조건 값 생성기(성명→가짜명단, 연락처→가짜번호, 날짜·학교·학년·금액·직위·과목·장소 12종) = **구성상 확정 정답**(PII 0·견본 혼동 0). `make_fill_dataset.py --synth`: 48종 파일럿에서 25종/75쌍(지시 3스타일: 나열·서술·축약, 쌍당 값 40 샘플 캡).
- **수확 경로의 교훈(보존)**: 단일 문서 휴리스틱으로 구조 셀(라벨·제목·단위·견본) 대 데이터 셀 구분은 정밀도 한계 — 대량 수집 후 **문서 간 신호**(같은 양식 여러 기관 → 공통 셀=구조)가 정공법. harvest 경로는 스크립트에 남겨둠.
...
- **검증 (vulkan 프로덕션 패리티 서버)**: ①공문: **관련일자 placeholder 5/5 방출**·실명 유출 0 — §37 학습 목표 달성 ②채움 held-out A/B: **v9 70% vs 베이스 21% (~3배)** — 양식 연습 데이터 가설 실증(전국 코퍼스에서 베이스는 약함) ③**계획서 혼선 의심**: '계획서 장·절' 지시에 v9이 공문체(1. 관련) 출력, 베이스는 정상 장·절 — v8과의 A/B 필요(채움 데이터 혼합이 계획서 축을 밀었을 가능성). ~~혼선 A/B + 간결 지시 확인 전까지 번들 채택 보류~~ → **A/B 결과 채택 확정(8/9 04시)**: 계획서 혼선은 **v8도 5/5 동일**(기존 결함, v9 회귀 아님 — v10 백로그: 계획서 업웨이트 강화). 간결 지시 서식완결 v8=v9=3.0/3.0. 모든 축 v9 ≥ v8 → **v9 번들 채택** (`b5e9f6f`): stage_bundle LORA/local_lora=v9 + dev settings + **NSIS 92.7MB·포터블 3.66GB 재빌드·스모크 통과**(nodes 31, feedback·fill-live/execute·preview 라우트 확인). stage_bundle에 구 어댑터 프루닝 추가(v8이 bundle/ 누적으로 NSIS까지 실려간 실측 — 재생성으로 v9 단독 확정).
...
### §37 관련일자 환각 방어 + 연도 무의존 확인 ✅ (2026-08-09 새벽, 사용자 라이브 테스트 피드백)
- **사용자 실측(Tailscale 라이브 점검 중)**: 관련 조항 `(2024. 10. 7.)` 날짜 환각 지적 + "연도 바뀌면 재학습?" 질문.
- **관련일자**: 모델이 알 수 없는 값 → placeholder 원칙 확장. ①후처리 `_verify_related_date` — 관련 줄의 날짜가 사용자 프롬프트에 실재할 때만 유지, 아니면 `(○○○○. ○. ○.)` (본문 날짜는 불변). 프롬프트의 행사 일시를 관련일자로 자동 대입하지 않음(그것도 조작). ②make_dataset: 관련절 날짜 → `{관련일자}` placeholder (다음 학습부터 모델이 직접 냄, 후처리 배선 완료) ③few-shot 예시도 ○ 표기로 교체 + 금지 규칙 명문화.
- **연도**: 재학습 불필요 — `{{오늘}}/{{올해}}/{{오늘한글}}` 런타임 주입이 이미 있음(llm_generate). raw 테스트 서버는 주입이 없어 2024가 보인 것. 원칙: **연도 인식은 하네스 몫, 모델은 무연도**.
- 오프라인 126 PASS(관련일자 5케이스). 라이브 점검 서버: CUDA bin 빌드 :8401(방화벽 Allow 확인), v8 스케일 1.0.

### §38 중첩표·병합 실측 확정 ✅ (2026-08-09 새벽, 사용자 질문 → 즉석 벤치)
- **질문**: "표병합·표 안에 표 복잡 양식도 가능?" → 코퍼스 실측으로 답변.
- **병합**: 설계 핵심(cellAddr/cellSpan 점유 맵) — 기존 벤치·실전 50종에서 검증 완료.
- **중첩표**: 48종 중 13종 보유. 1차 왕복 50.8%로 보였으나 **테스트 인공물**(헤더 구간 셀은 채움 제외가 설계, 콜론·괄호 슬롯은 클리어 금지가 설계 — §32 ② 보호가 정상 작동한 것). 데이터 셀만 고른 재측정: **왕복 426/427 = 99.8%**. 중첩 셀ID 체계(파스-필 공유 순회)가 중첩에서도 일관함을 확정.
...
### §41 E4B v9 A/B — 채택 보류 (2026-08-10 새벽, 사용자 요청 산출물 전송 완료)
- **학습**: E4B 519스텝 **9h49m 완주**(QLoRA 4bit, checkpoint-196에서 재개 — bf16은 272s/it 스필로 포기, 4bit 전환 후 100~127s/it). loss 0.36. → `gongmun_g4e4b_v9.gguf`(140MB).
- **A/B 실측 (동일 v9 데이터·동일 조건 vulkan+reasoning off)**:
  | | E2B v9 (Q4_K_M) | E4B v9 (Q3_K_S) |
  |---|---|---|
  | 겉공문 서식완결 | 2.75/3 | **3.00/3** |
  | 평균 항목 수 | **2.5개** | 1.8개 |
  | 평균 길이 | **458자** | 236자 |
  | 채움 held-out | **70%** | **42%** |
- **판정: E4B 채택 보류.** 모델을 키웠는데 채움이 오히려 28%p 하락하고 생성 내용도 짧아졌다 — "크면 낫다"가 성립하지 않음.
- **원인 후보(미검증)**: ①**양자화 비대칭** — 배포용 E4B는 **Q3_K_S(3비트)**, E2B는 Q4_K_M(4비트). 3비트 손실이 크기 이득을 상쇄했을 가능성이 가장 유력. ②QLoRA(4bit 베이스) 학습이 어댑터 품질을 깎았을 가능성. → 재검증하려면 **E4B Q4_K_M GGUF 확보 후 재측정**이 먼저(모델 파일만 바꾸면 되는 저비용 실험).
- **산출물**: `D:\lora_train\e4b_compare\E2B_vs_E4B_공문비교.pdf`(8종 나란히, 제품 후처리 적용) — 텔레그램 전송 완료. 스크립트 `scratchpad/e4b_compare.py`(--stage e2b|e4b|render), `scratchpad/send_tg.py`.
- **교훈**: 모델 크기 실험은 **양자화 등급을 맞춰야** 유효하다. 이번은 크기와 양자화가 교란돼 순수 크기 효과를 못 쟀다.

### §41b 교란 제거 재측정 — E4B 열세 확정 (2026-08-10 05시, 자율)
- **Q4_K_M 제작**: `convert_hf_to_gguf.py`로 E4B F16(15.1GB, E:\sbc_lab\gguf_work) → `llama-quantize Q4_K_M` **5.34GB** 확보 → E2B와 양자화 등급 일치.
- **측정 병리 발견 → 지표 재정의**: 파싱 실패 시 분모에서 빠져 **정확도가 부풀려지는** 결함 발견(E4B Q3 "42%"는 성공분 35필드만의 값). → **엄정 지표 = 전 문서 gold 필드 전체를 분모로, 파싱 실패는 0점**(엔드투엔드 = 사용자가 실제로 얻는 값).
- **동일 프로토콜 결과** (Q4_K_M · reasoning off · **--reasoning-budget 0** · mt 3000 · 8문서 123필드):
  | | 엄정 정확도 | 파싱 실패 |
  |---|---|---|
  | **E2B v9** | **38/123 (31%)** | **2/8** |
  | E4B v9 | 13/123 (11%) | 6/8 |
- **판정: E4B 최종 탈락.** 양자화를 맞춰도 E2B의 1/3. **원인은 크기가 아니라 신뢰성** — E4B는 스키마 강제 채움에서 **사고 토큰이 예산을 전부 소진**(실측: reasoning_content 5,482자 / content 0자, finish=length)해 답 자체를 못 낸다. `--reasoning-budget 0`으로도 6/8 실패 — gemma-4 E4B의 thinking 억제가 이 빌드에서 불완전.
- **부수 발견(중요)**: `--reasoning off`만으로는 사고 토큰이 안 잡힌다 → **`--reasoning-budget 0` 병행 필요**. 다만 E2B는 이 옵션 유무와 무관하게 안정적이라 현행 배포 경로는 영향 없음. **JSON 스키마 강제 + thinking 모델 = 예산 소진 위험**은 일반 함정으로 기록.
- **결론**: **E2B v9 단일 배포 유지**. "고품질 팩" 구상은 E4B로는 성립 안 함 — 대안은 ①다른 4B급 비-thinking 모델 ②E2B 데이터·학습 개선. 산출물 E4B Q4_K_M GGUF는 E:에 보존(재실험용).
- **주의**: 위 31%는 §35b의 "70%"와 다른 지표다(70%=성공 문서 내 필드 정확도, 31%=파싱 실패 포함 엔드투엔드). 앞으로는 **엄정 지표로 통일**한다.

### §41c 채움 오류 분해 → 실제 결함 2건 수정 (2026-08-10 06시, 자율)
- **동기**: §41b에서 드러난 엄정 채움 31%의 내막을 오류 유형별로 분해.
- **분해 결과 (E2B, 123필드)**: 정답 41 · 값불일치 6 · 누락 3 · **문서 통째 실패 73(59%)** — 즉 31%의 대부분은 "틀린 답"이 아니라 **단 2개 대형 문서(33·40빈칸)에서 응답 자체를 못 받은 것**.
- **근본 원인(실측)**: 소형모델이 **같은 셀ID를 반복 출력하며 루프** — 한 응답에서 19항목 중 고유 12개, 한 ID가 7회. max_tokens를 소진해 절단 → 응답 전멸.
- **결함 2건 수정** (`engine/form_assist.py`):
  1. **스키마에 `maxItems`가 없었다** → 배열이 무한히 길어질 수 있었음. `maxItems=빈칸 수`로 문법 차원에서 루프 종결.
  2. **파서가 중복 id에서 마지막 값 채택** → 루프 쓰레기가 정답을 덮어씀. **첫 값 채택**으로 변경.
  3. 추가: **필드 수 청킹** `_GRID_CHUNK_FIELDS=15` 신설 — 문자 예산(7000자)만으로는 안 걸리는 **밀집 양식**(3.5천자에 빈칸 33·40개)에서 모델이 중도 포기하던 문제. 같은 표는 렌더를 공유하며 필드만 분할.
- **효과 (동일 held-out 123필드)**: **응답 실패 2/8 → 0/8**(전멸 제거), 정확도 31% → 33% → **36%**(청킹까지).
- **가장 중요한 발견 — 36%는 두 집단의 평균**:
  | 양식 유형 | 정확도 |
  |---|---|
  | 소·중형(6~12빈칸) 6종 | **40/50 = 80%** |
  | 대형 명단형(33·40빈칸) 2종 | 4/73 = **5%** |
  - 대형 명단형 실패는 **셀-값 정렬 문제**: 지시문에 인명 40개가 나열되고 모델이 각 행에 배치해야 함(실측 오류가 전부 `임채원↔박지호` 식 **이름 뒤섞임**). 의미 매칭이 아니라 정렬·기억 과제라 소형모델에 구조적으로 불리.
  - **처방은 이미 §39에 있다: `fill_column`**(라벨+값 배열로 열 단위 일괄 채움). 명단은 셀 하나씩이 아니라 열로 채워야 정렬이 코드 몫이 된다. → **v10/§42 1순위로 승격**.
- 오프라인 130 PASS. **주의: 벤치 자체의 한계** — 합성 데이터가 무작위 인명을 무작위 셀에 배치해, 실사용(명단 파일 첨부·열 단위 입력)보다 어렵다. 대형 명단 점수는 하한으로 읽을 것.

### §41d 합성 데이터 오염 3종 발견·제거 — v10 재학습 재료 확보 (2026-08-10 06시, 자율)
- **경위**: §41c에서 "대형 명단형 5% → fill_column이 처방"이라 적었으나, **문서 구조를 직접 열어보니 틀린 진단**이었다. 실패 문서는 명단이 아니라 **합성 데이터 자체가 풀 수 없게 만들어진 것**이었다.
- **오염 3종 (v9 학습 데이터 실측, 셀 4,886개 기준)**:
  1. **라벨-값 모순 8%** — 라벨이 `1학년`(열 헤더)인데 정답이 `6학년`. `_LABEL_GEN`의 `학년` 규칙이 "학년을 묻는 칸"과 "학년이 적힌 헤더"를 구분 못 함.
  2. **구분 불가 42%** — 서로 다른 셀이 **같은 라벨**을 갖는데 무작위 값이 각각 달라, 모델이 원리상 어느 칸에 무엇을 넣을지 알 수 없음.
  3. **목록 밖 정답 17%** — 프롬프트 빈칸 목록이 120개로 잘리는데 정답에는 잘린 칸이 포함. **알려주지도 않은 칸을 맞추라고 요구.**
- **수정** (`make_fill_dataset.py`): ①`학년` 규칙에 `(?<!\d)학년(?!도)` 경계 ②중복 라벨 셀 전량 배제 ③`render_blank_grid`가 **프롬프트에 실린 셀ID 집합을 반환**하고 정답을 그 집합으로 필터.
- **결과**: 목록밖 17%→**0%** · 구분불가 42%→**0%** · 라벨모순 8%→**0%** (셀 4,886→2,151 — 절반이 학습 불가였다는 뜻).
- **판정 정정**: §41c의 "fill_column이 1순위" 결론은 **근거 부족으로 철회**. 대형 명단형 5%는 제품 결함이 아니라 **벤치 결함**이었다. fill_column은 여전히 유용하나(§39) 우선순위 근거는 재확보 필요.
- **다음(v10)**: 정화된 채움 데이터(2,151셀)로 재학습 → 채움 정확도 재측정. **v7 HTML표 사건과 동형** — 데이터 정화가 학습·구조 개선보다 레버가 크다는 세 번째 사례.
- ⚠️ **현행 v9 어댑터는 이 오염 데이터로 학습됨** — 채움 성능의 상당 부분이 노이즈 학습에 눌려 있을 가능성. 재학습 가치가 높다.

### §41e v10 학습 착수 → VRAM 스필로 중단, 재부팅 대기 (2026-08-10 08시)
- **착수**: §41d 정화 데이터로 v10 학습 실행. 재료 검증 전부 통과 — `dataset_v10`(train 2,829/val 148, 형식 정상) · venv(torch 2.11.0+cu128, tf 5.13.1, peft 0.19.1) · 베이스 `D:/models/hf/gemma-4-E2B-it` · 디스크 D: 9.4GB(`save_total_limit=2` LoRA라 ~2GB 미만 사용, 충분).
- **기동 정상 확인**: trainable 24,158,208 / 5,128,455,712 = **0.47%**(rank16 정상), target_modules가 `language_model` 한정으로 정확히 잡힘, **train 2,732 / val 143**(max-len 2048 초과 97+5 스킵), 총 **513스텝**(2732÷16×3).
- **중단 사유 = VRAM 스필**: s/it가 8.4 → 19 → 24 → 29초로 **단조 상승**, VRAM 99%(15,853/16,303MiB) 고정. ETA 1h11m → 4h03m로 발산.
- **범인 = 종료 불가 orphan `llama-server.exe`(PID 24952)**: 부모 프로세스(9656) 이미 사망 · `:8407`(엔진)·`:8400` **둘 다 미개방 = 활성 서비스 아님** · 그런데 **VRAM 2.8GB 상주**. `taskkill /F`·`Stop-Process -Force` **둘 다 "액세스 거부"**(상위 권한 컨텍스트) → 유저 권한으로 회수 불가.
- **`expandable_segments:True` 시도 → 무효**(6.97 → 20.7 → 27.8s/it 동일 발산). ⇒ **단편화가 아니라 실제 메모리 압박**. E2B bf16 학습은 **여유 12.87GB로는 부족**하고 16GB 전체가 필요하다는 뜻(v9도 동일 레시피로 1h20m 완주했었음 = 그때는 GPU가 깨끗했음).
- **조치**: 학습 프로세스 정리(체크포인트 생성 전이라 **손실 없음**), **사용자 재부팅으로 orphan 회수**. 재부팅 후 아래 명령 그대로 재실행.
- **교훈(신규 함정)**: ①**학습 전 `nvidia-smi`로 유휴 VRAM을 먼저 확인**하라 — 순간 util%가 아니라 **memory.free**가 지표. ②orphan llama-server는 부모가 죽어도 VRAM을 놓지 않으며 **권한 상승 없이는 못 죽인다** → 재부팅이 유일 해법. ③**s/it 단조 상승 = 스필의 조기 신호**(첫 5스텝만 봐도 판별 가능, 4시간 낭비 전에 잡을 수 있음). ④`expandable_segments`는 단편화용이라 순수 용량 부족엔 무효 — 둘을 구분하는 진단이 된다.

### §41f E4B 사면 복권 — "탈락" 원인 3층 해부, 베이스가 채움 최강 실증 (2026-08-10 저녁)
- **동기**: 사용자 "E4B도 잘 살리고 싶다" + 컨텍스트 질문 → **config 실측: E2B·E4B 둘 다 131,072(128K) 동일**(sliding_window 512, 레이어 35/42, hidden 1536/2560). E4B의 가치는 컨텍스트가 아니라 용량. KV캐시는 E4B가 커서 같은 VRAM에서 실용 컨텍스트는 오히려 짧다.
- **llama.cpp b10338 격리 테스트**(`E:\sbc_lab\llama_test\b10338\`, 기존 b10298 무손상): §41b "사고 토큰이 응답을 삼킨다"의 진짜 정체를 3층으로 분해:
  1. **스키마 경로가 thinking을 깨움**: 템플릿 조건 `enable_thinking or tools or system-role` — json_schema 요청이 system 블록을 주입해 사고 채널 활성화. **b10338 + `chat_template_kwargs:{"enable_thinking":false}`로 무력화**(doc0: 0자→17/17 만점). **b10298은 이 kwargs를 조용히 무시**(적용 안 됨 실측) → 어댑터-ON 생성 보호에는 번들 업그레이드 필요.
  2. **진짜 범인 = v9 E4B 어댑터**: 실패 문서의 reasoning_content 실물 = thinking이 아니라 **공문 생성문 무한 루프**(`1. 관련: {기관명}-{문서번호}...` + `<|channel>` 마커 반복). 오염 데이터(§41d)로 학습된 어댑터가 **채움 과제를 공문 생성으로 오라우팅**, 채널 마커가 reasoning 파서에 걸려 문법 트리거 미발동. 어댑터 off 시 같은 문서 2/2 만점. logit_bias(`<|think|>`=98 금지)로도 안 잡힘 — thinking이 아니니까.
  3. 양자화(§41b Q3→Q4)는 부차 요인.
- **4원 엄정 벤치**(v10 val 채움형 8문서/90필드, 동일 프로토콜: b10338·kwargs끔·스키마 강제·파싱실패=0점):
  | | v9 어댑터 | 베이스 |
  |---|---|---|
  | E4B | 50% (실패 5/8) | **69% (실패 0)** |
  | E2B | 54% (실패 1/8) | 56% (실패 0) |
  - **E4B 베이스가 채움 최강** — §41b "E4B 탈락"은 어댑터+스키마 병리의 오진. 크기는 죄가 없었다.
  - **v9 어댑터는 채움에 순손해**(양쪽 모두) — §41d 오염의 실증. 정화 v10 재학습의 근거 완결.
  - ※ 이 90필드 지표는 §41c의 123필드(오염 포함 v9-era)와 분모가 다름 — 상호 비교 금지, 매트릭스 내부 비교만 유효.
- **엔진은 이미 정답 배선**: 어댑터 scale 0 프리로드 + 생성만 요청별 1.0, 채움·추출은 베이스(llm_manager.py). 콜론 문법(`FNAME:SCALE` cwd 트릭)도 기구현 — **드라이브 콜론(`D:`) 경로는 b10338이 파싱 불가**이므로 상대경로 유지 필수.
- **함정 기록**: b10338에서 `--lora-scaled` 콜론 단일인자 형식(구 2-인자 불가) · 신규 `--reasoning-budget-message` 플래그 존재 · 벤치 스크립트 `scratchpad/strict_fill_bench.py`(포트·라벨·lora스케일 인자).
- **시사점**: ①16GB 교사 PC용 "고품질 팩" = **E4B 베이스 채움**은 현행 번들로도 즉시 유효 ②E4B 공문 생성(어댑터 필수)은 v10 정화 재학습 + b10338 업그레이드 후 ③E2B v10 어댑터가 베이스(56%)를 넘는지가 재학습 평가 기준.

### §41g v10 무인 학습·벤치 체제 가동 (2026-08-10 밤 — 사용자 24h 부재 대응)
- **사용자 지시**: "알아서 진행하고 중간결과를 텔레그램으로 계속 보내라."
- **체인** `E:\sbc_lab\tf_build\v10_chain\chain.py` (분리 프로세스, 20:37~): E2B bf16 → GGUF → E4B QLoRA → GGUF. **스필 자동감지**(초기 스텝 s/it 프로브, E2B>15s/it·E4B>180s/it면 킬→30분 후 재시도 — 데스크톱 유휴 대기) + VRAM 게이트 + checkpoint-* 자동 재개. 마커 = DONE.txt/FAIL.txt.
- **리포터** `reporter.py` (분리 프로세스, 21:08~): 5분 폴링, **2시간 간격 텔레그램 진행보고**(cokacdir_desktopbot→owner, `send_tg.py`) + E2B v10 GGUF 등장 시 **자동 벤치**(b10338 서버에 v9+v10 어댑터 동시 프리로드 → 채움 엄정 3구성(베이스/v9/v10) + 공문 생성 A/B 자동점검(PH·결문·항목수)) → E4B 완료 시 동일 벤치 + 최종보고. 실패 즉시 경보. 상태 = reporter_state.json.
- **스필 원인 확정(§41e 갱신)**: venv(7/12)·드라이버(5/27) 불변 + v10 데이터가 v9보다 짧음(mean 562 vs 653 토큰) → 남는 유일 설명 = **v9는 새벽 유휴에 돌았고 지금은 데스크톱 활성**(Chrome·Claude 등 WDDM 동적 경합). bf16@2048은 슬랙이 수백 MB뿐이라 낮에는 불가. 로짓 스파이크(vocab 262K×2048×fp32 ≈ 4GB)가 경계 요인.
- **운영 중 보완(23시)**: ①**max-len 단계 폴백** — 시도 1~4=2048(레시피 순수), 5~7=1792(-7샘플), 8+=1536(-26샘플, 1%) → 데스크톱이 계속 활성이어도 완주 보장 ②chain·reporter에 **싱글턴 락**(msvcrt) ③**함정: Windows venv python.exe는 런처** — 베이스 인터프리터(Python310)를 자식으로 띄우므로 프로세스 목록에 "venv 부모+PY310 자식" 쌍으로 보임. 중복 실행 오인 금지(이 오인으로 리포터를 한 번 오살했음).
- **사용자 복귀 후 할 일**: ①텔레그램 벤치 결과로 **배포 반영 결정**(채움: v10 어댑터 > 베이스면 승, 아니면 채움=베이스 유지 / 공문: v10이 v9 대체할지) ②채택 시 `packaging/stage_bundle.py` LORA·local_lora v10 교체 → 재빌드 체인(§28) ③번들 b10298→b10338 업그레이드 결정(§41f — kwargs 지원 필요시).

### §41h v10 완결 — 확정 매트릭스·스위칭 아티팩트·최종 권고 (2026-08-11)
- **학습 완주**: E2B v10(bf16, **max-len 1536 폴백** — 2048·1792 7회 스필 후, 1h27m) + E4B v10(QLoRA 2048, 1시도 9h13m, 65s/it — worker 종료 덕에 v9 때보다 1.7배 빠름). GGUF 2종 `D:\models\loras\`.
- **⚠️ 측정 함정 (신규 확정)**: "어댑터 2개 프리로드 + 요청별 scale 전환" A/B가 **b10338에서 불안정** — 같은 E2B v9가 54%(고정)→28%(스위칭)로 요동, 요청별 scale 0의 "베이스"도 서버에 실린 어댑터에 따라 50~56% 오염. **어댑터 A/B는 고정 프로토콜(서버당 0~1개 고정 탑재, 요청 lora 필드 금지)로만 측정한다.** §38 프리로드 A/B 관행 폐기.
- **채움 확정 매트릭스** (90필드·고정 프로토콜): E2B 베이스 53/v9 54/**v10 44** · E4B **베이스 62**/v9 31/**v10 50**.
  - 정화 효과 실증: E4B 31→50%, 파싱실패 6→3 (오라우팅 병리 대부분 해소). 그러나 **베이스를 넘는 채움 어댑터는 없음**.
  - E2B v10은 v9보다 후퇴(44<54) — E2B급 채움 어댑터 학습은 접근 재고(v11에서 채움 태스크 제외 검토).
- **공문 생성 A/B**: E4B v10 **3/3 완벽**(PH·결문) vs E4B v9 발산 1/3 · E2B v10 붙임 반복 병리 1/3 vs E2B v9 양호.
- **최종 권고**: ①채움 = 베이스(엔진 기본값 그대로) ②**E2B 배포 = v9 유지** ③**E4B 고품질팩 = 베이스(채움 62%) + v10 어댑터(공문)** — 단 번들 b10338 업그레이드 전제(§41f) ④사용자 복귀 후 배포 반영 결정.
- 운영 사고 2건 자동 복구: 새벽 DNS 장애 TG 3건 유실(→재시도+`tg_outbox.log` 보존 패치) · E2B 벤치 연기 버그(→완료 후 실행 패치). 보고서 PDF 텔레그램 전송 완료(15:00).

### §41i Opus 대조군 + 공문 산출물 고정 재생성 — 정정 2건 (2026-08-11 오후)
- **Opus 채움 대조**(인세션 하위에이전트 8병렬, 동일 90필드·gold 차단): **90/90 = 100%, 파싱실패 0**. 의미: ①정화 벤치는 원리상 전부 가해(§41d 완결의 외부 증명) ②로컬 최강(E4B 베이스 62%)과 38%p 격차 = 남은 몫은 구조 하네스(fill_column 등 §39)+모델 세대 ③비용·PII·오프라인 우위는 불변.
- **공문 5구성 전문 재생성**(고정 프로토콜, 3과제×5): E4B v10 **3/3 무결점** · E4B v9 과제2 발산(1,111자, 재현 확정) · **E2B v9·v10 둘 다 3/3 준수**.
- **정정 ①**: §41h의 "E2B v10 붙임 반복 병리"는 **스위칭 서버 아티팩트** — 고정 프로토콜에서 미재현. E2B v10 공문은 정상. E2B 배포 권고는 "v9 유지, v10은 라이브 검증 후 교체 검토"로 완화.
- **정정 ②(발견)**: **Opus는 placeholder 규율 없이 관련 문서번호를 창작**("감사관-1234" 등) — 프론티어도 규율 없으면 실무 위험. 로컬 어댑터의 `{기관명}-{문서번호}` 방식이 안전하다는 실증 = 어댑터의 실질 우위 논거.
- 산출: `scratchpad/gongmun_outputs.json`(5구성 전문)·`opus_answers.json`·PDF 2종 텔레그램 전송 완료(모델별_산출물_비교.pdf).

### §41j 반복 루프 방어 확장 — DRY 전면화 + length 재시도 (2026-08-11 저녁)
- **원리(사용자 질의로 정리)**: 소형모델 반복 = 복사 회로(induction)는 세고 의미적 진행 판단은 약한 체급 성질. 불확실성↑ → EOS 확률↓ → 직전 패턴 복사 자기강화(temp 0은 탈출 무작위성 없음). v9 오염→루프, v10 정화→소멸이 실증.
- **엔진 반영** (`llm_manager._local_chat_completion`): ①DRY(0.8/4)를 어댑터-ON 한정 → **비스키마 자유 생성 전체로 확장**(무어댑터 채팅도 영향권; JSON 채움·추출은 문법이 차단+정당 반복 구조라 제외 유지) ②**finish=length 감지 시 1회 루프브레이크 재시도**(temp max(t,0.4)+DRY 1.2, 소예산<256 제외) → 그래도 length면 자유 텍스트만 꼬리 미완 행 제거(JSON은 원형 유지, 상류 관대 파싱 담당).
- **검증 3종 통과**: 오프라인 스위트 PASS · **채움 495/495 회귀 100%** · 공문 스모크(정상 2건 PH✓결문✓반복무 + max_tokens 300 length 유도 → 재시도가 **예산 내 완결본 재구성** 반환).

### §42 채움 고도화 Stage 1 — 결정적 사전 매칭 + k-표결 (2026-08-11 밤)
- **설계 확정(사용자)**: 베이스는 **4B 이내 고정**(교사컴 현실) — 경로 = 하네스 → Opus 증류 → RL(GRPO). 7~8B 후보 평가는 제외.
- **구현** (`engine/form_assist.py`): ①`prematch_fields` — 지시문의 명시 라벨→값 쌍을 코드가 선채움. 은(는) 나열형 + 콜론형(한 줄 다중 쌍, 시간 `10:00` 콜론 가드, `::` 병합, 날짜값 ". " 보존) 파싱 → 정규화 완전일치·포함일치 각 **후보 유일**일 때만 채움(중복 라벨=모호→LLM). 오추출 쌍은 라벨 매칭 실패로 무해(내장 안전핀). `fill_prematch`(기본 on) ②**k-표결** — `fill_votes`(기본 1)>1이면 temp 0.6 k회 셀별 다수결 ③사전 매칭분은 LLM enum에서 제외(프롬프트 축소)·병합 시 항상 우선.
- **실측(90필드 val)**: 선채움 **52/90(58%) 오배치 0** → 전체: 베이스 62% → **선채움+베이스 72% → +3표결 74%** (+12%p, 학습 0). 회귀: 오프라인 PASS·495/495 유지.
- **다음**: ①평가셋 확장(fill_v10/blank_forms가 비어 있어 make_fill_dataset 재생성 필요 — 90필드에 과적합 방지) ②fill_column(명단형, 잔여 오류의 주범 docs 1·2·6) ③Opus 증류 데이터 생성(Mac1 크롤 자산) ④GRPO.

### §42b 확장 검증 + 증류 준비 (2026-08-11 밤 계속)
- **확장 평가셋**: `make_fill_dataset --synth --seed 777` → 54문서/**616필드**(6.8배). 벤치 `scripts/lora/fill_harness_bench.py`.
- **Stage1 확정(616필드)**: 베이스 50% → **선채움+베이스 83%(+33%p)** → +3표결 82%(무익 → fill_votes 기본 1 확정). 선채움 정밀도 94%(431/454) — 90필드의 100% 대비 오배치 23건, 정밀 보강 과제.
- **교사 자격**: Opus 100% + **Sonnet 100%**(90필드) — 증류 교사 = Sonnet 확정(비용↓ 볼륨↑, 사용자 조건 "100% 아니면 탈락" 통과).
- **증류 v1 파이프라인**: 정답=코드 합성(gold 검증가능) + **지시문만 Sonnet 6스타일 다양화**(구어/메신저/이메일/메모/서술/명단첨부, 값 전수 포함 규칙 → 코드 재검증, 실패 시 원본 폴백). 시드 1001~1003 × 48양식 = 162쌍 1차. 다음 볼륨: 미변환 hwp 85건 + Mac1 크롤 양식.

### §42c 증류 v1 데이터 완성 (2026-08-11 심야)
- **162쌍 조립 완료** `D:\lora_data\distill_v1	rain.jsonl` — 값검증 3셋 전량 54/54 통과(값 누락 0). 지시문 = Sonnet 6스타일 다양화(s1003은 에이전트가 템플릿 방식 사용 — 산문 다양성은 s1001·1002 대비 낮음, 차기 배치에서 산문 강제).
- **다음 세션**: ①볼륨 확장(미변환 hwp 85건 → convert_hwp_batch, Mac1 크롤 양식 rsync, 시드 추가) ②500쌍+ 모이면 E4B 채움 전용 어댑터 학습(공문과 분리, r32) → 616필드 A/B(목표: 하네스 83% 위에 +) ③선채움 오배치 23건 정밀 보강 ④fill_column.

### §42d 증류 v11 — 볼륨 확장 + 학습 착수 (2026-08-12 새벽)
- **볼륨**: Mac1 form_attach 172파일 수급 → hwp 136건 COM 변환(136/136) → 신규 양식 142(학습)/30(평가 held-out, 시드 3000 `fill_eval_unseen`) 분할. 학습 풀 = 구양식 9시드(486) + 신규 2시드(390) = **876쌍**, Sonnet 다양화 270쌍 적용(값검증 통과분만, 나머지 정형 = 스타일 혼합).
- **선채움 정밀 보강**: 연결 오염 가드(값 속 타 라벨 기각) + 라벨 후미 재절단 — 95%. **라벨 주도 스캔은 시도 후 롤백**(커버리지↑였으나 정밀 86%로 붕괴 — 선채움은 정밀 우선 원칙 확정).
- **학습 중**: `fill_g4e4b_v11`(E4B QLoRA r32 2ep, 채움 전용 — 공문 어댑터와 분리). 완료 후 GGUF → **미학습 30양식 A/B**(베이스+하네스 vs v11+하네스)가 최종 판정.

### §42f 채움 트랙 완결 — v12 청크 어댑터 94% 채택 (2026-08-12 정오)
- **최종 3파전(미학습 30양식/364필드, 고정 프로토콜)**: 베이스 56/86/88 · v11(문서통째 365쌍) **3**/79/81 · **v12(청크 1,540쌍) 81/94/94** (단독/+선채움/+표결).
- **판정**: ①**v12 채택** — 채움 신기록 **94%**(Opus 100%와 6%p차, 출발점 50%) ②**청크 학습 = 장문 극복의 정답 실증**(같은 재료에서 v11 3% vs v12 81% — 학습-추론 분포 일치가 전부) ③v11 폐기(과적합 붕괴의 교보재) ④선채움 정밀 99% 유지.
- **학습 여정 함정**: E4B QLoRA 주간(데스크톱 활성) 학습은 r32@2048=730s/it, r16@2048도 스필 → **r16@1536**(1,540/2,040샘플)이 주간 실용 프로필. 야간이면 가속(11:01 조기완주).
- **산출**: `D:\models\lorasill_g4e4b_v12.gguf`(140MB) · v12 조립기 `scripts/lora/make_chunked_fill_dataset.py` · 벤치 `fill_harness_bench.py` · 다양화 519건(값검증 전량 통과).
- **남은 통합(다음 세션)**: ①엔진 채움 경로에 fill 어댑터 배선 — 현재 엔진은 채움=어댑터OFF가 기본. 설계: settings `fill_lora` 신설 → llama-server에 공문·채움 어댑터 2개 프리로드 → 생성=공문 1.0, 채움=fill 1.0 요청별 전환(§41h 스위칭 함정 재검증 필수 — 단일어댑터 고정이 안전하면 채움전용 서버 프로필 고려) ②거대 표 행-윈도 렌더(v13 — 학습·런타임 공통) ③m2001 잔여 다양화 반영 v12.1 ④GRPO.

### §42h 확장 검증 매트릭스 + 오류 해부 — 24h 트랙 완결 (2026-08-13 오전)
- **매트릭스(미학습 30양식, 정형 3시드 1,008필드 + 자유문장 364필드)**:
  | | 정형 종합 | 자유문장(교사말투) |
  |---|---|---|
  | 베이스 | ~82% | **76%** (선채움 무효) |
  | v12+하네스 | 94/88/87-89 (종합 ~91%) | **91-92%** |
  | v13+하네스 | 94/91/86 (종합 ~91%) | 89-90% |
- **판정: v12 유지** — v13(행-윈도 학습)은 동률(스필로 학습조건 불리·1536 절단 영향 추정). 행-윈도 렌더 자체는 런타임 개선으로 유지(§42g). v13 어댑터는 보존.
- **대발견 ①**: 증류 어댑터의 이득이 **자유문장에 그대로 이전**(베이스 76→v12 92, +16%p) — 정형 합성으로 배운 기술이 실전 말투에 일반화됨을 실증.
- **대발견 ②**: **선채움 파서가 자유문장에서 0건**("성명에 정도윤, 연락처엔…" 조사형 미지원) — 파서 조사형 확장 시 자유문장도 94%+ 여지 = 다음 최대 레버.
- **오류 19건(v12·s3000) 해부**: ①라벨 에코 ~6건(예측=라벨 그대로 — 후처리 기각·재질의로 회수 가능) ②예시 혼입 ~5건("(예시)4.8.(수)"를 값으로 복사 — 라벨의 예시 세그먼트 클린으로 회수 가능) ③진짜 정렬 실수 ~8건(반복 구조 다중 매핑 — RL·승인 패널 몫). **①+② 회수 시 ~97% 도달 가능, 잔여 ~3%는 확신도 승인으로 시스템 차원 0화.**
- **100% 로드맵 확정**: 조사형 선채움 → 라벨에코·예시 후처리 → (97%) → 확신도 낮은 셀만 승인 패널 → 체감 100%. 모델 재학습은 GRPO 1개 카드만 남음.
- 운영 실수 기록: v13 완료(00:16) 후 후속 트리거 누락으로 8h 정지 — 체인형 자동화(완료→변환→벤치 일체형)가 워처+수동 조합보다 옳다는 재확인.

### §42i 100% 작업 1차 + 실물 검증 (2026-08-13 오전)
- **라벨 에코·예시 복사 기각** 후처리 채택(`_plan_grid_fill` 후필터: 예측=라벨 or 6자+ 라벨 포함 → 기각·로그) → s3000 **95%**(표결 구성 346/364, +1). 기각분은 현재 미채움 처리 — 점수 전환은 재질의(retry) 구현이 다음 카드.
- **자유문장 라벨-스캔 기각**: 5/5 오배치(자유문장은 라벨을 바꿔 말함 — 리터럴 매칭 불가). 의미 매칭은 LLM 몫 확정. 함수는 참고용 보존(`_scan_freeform_pairs`, 미호출).
- **실물 검증 완료(사용자 요청)**: 미학습 양식 3건을 제품 주입 코드(`fill_hwpx_cells`)로 실제 완성 hwpx 생성 → 한/글 COM SaveAs("PDF") 전/후 6건 텔레그램 전송. 소형=선채움만으로 3/3(LLM 0회)·대형 8/8·자유문장 3/3.
- **100% 현황**: 정형 95%(에코 기각 반영) · 자유문장 92%(어댑터 순수) · 잔여 카드 = 기각분 재질의 → 확신도 승인 패널 → GRPO.

### §42j 실물 전체채움 검증 + 중첩표 주입 붕괴 버그 수정 (2026-08-13 오후)
- **사용자 지적 2건 검증**: ①"뒤죽박죽 채움" = 합성 시나리오 결함(무작위 셀 표본 지시문 — 모델은 지시대로 채웠고 gold 100%)이었음을 비전 검수로 확정 ②충분한 맥락(운영대장 md) 제공 시 **전 페이지 채움 성립** — 4자 비교(Opus 91/Sonnet 99/Haiku 76/로컬 183) 5페이지 실물 PDF 텔레그램 전송.
- **제품 버그 발견·수정(중대)**: `fill_hwpx_cells`가 **중첩표 래퍼 셀**에 주입하면 내부 표가 밀려 **한/글 렌더 통째 붕괴**(4p→1p). kordoc validate·자체 파서는 통과하는 **렌더 전용 함정** — 래퍼 셀(tc 내부에 tbl 존재) 채움 금지 가드 추가(hwpx_grid), 자동 이분탐색으로 범인 셀 규명, 회귀 PASS. **비전 검수 게이트의 가치 실증**(발송 전 PDF를 눈으로 확인).
- **한/글 PDF 변환 함정**: 한 COM 세션에서 연속 SaveAs(PDF) 시 2번째부터 붕괴 사례 → **파일당 새 인스턴스** 원칙.
- **로컬 잔여 결함(비전 확인)**: ①라벨/제목 셀 침범 — 괄호형 슬롯 오분류("성명(기관장)" 류 라벨을 paren 슬롯으로 인식) ②본문 문단형 항목(서식3 보고서)은 빈칸 목록 미포함 — **표 밖 본문 채움**이 다음 기능 카드 ③서명란 라벨에코는 제품 가드 존재(벤치 스크립트가 우회했을 뿐).
- Opus 대조: 라벨 보존·체크박스·15행 완전 — 하네스+주입기 조합의 상한 확인.

### §42k Opus급 개선 실행 — 후보 정화 3종 (2026-08-13 오후)
- **처방→실행**: ①`_is_signature_field` — 서명·(인)·날인 칸 추출 단계 원천 제외 ②에코 가드 **양방향**("12×담당교사 서명" 류 = 라벨⊆값 차단) ③벤치 입력의 제품 필터(`_is_fillable`) 우회 결함 수정 + 청크 프롬프트에 "서명·제목 칸 금지" 규칙.
- **효과(비전 검수)**: 채움 후보 189→**82 정화** → 라벨·제목 침범 소멸, 서명란 깨끗, 강의표 15행 중 14행 완벽. **격차의 대부분은 지능이 아니라 후보 정화였음**(Opus가 판단으로 하던 것을 코드가 대신).
- **잔여 격차(문서당 ~2건)**: 윗행 값 반복·행 누락 = 순수 의미 정밀 → 증류 v14(부정 예제 포함)·GRPO·확신도 승인 카드 몫.
- 오프라인 PASS. 산출 `전체채움_local_v2.pdf` 텔레그램.

### §42l 완결성·타입·견본 3종 + 행 정렬 한계 확정 (2026-08-13 저녁)
- **제품 반영 3종**: ①**완결성 재질의**(미채움 후보 1회 재질의 — 행 누락 구조 방어, `fill_retry` 기본 on) ②**타입 검증**(`_type_suspect`: 연락처=전화꼴·날짜/인원/금액=숫자·이메일=@ — "연락처에 이름" 기각) ③**견본혼합 감지**(`_is_placeholder` 확장: ○류 2자+ or 전체 날짜스캐폴드 — §32 백로그 해소. (요일) 부분매칭은 헤더 라벨 오인 FP로 fullmatch로 정밀화). 회귀: 오프라인 PASS·495/495.
- **1행 누락 진범** = 견본 행("대전○○초"·"정○○")을 기채움으로 오판해 후보 제외 → 감지 확장으로 후보 82→88 회복.
- **잔여 한계(비전 v5)**: 견본 행 섞인 표의 1~5행 정렬 혼선(순번을 날짜에 등) = 소형모델 행 앵커 한계 — **v14 학습 몫**(견본행 교체 예제 + 완결 행 채움 예제 합성이 차기 데이터 카드).
- v14 1차(연속 학습: v13셋 2ep) 야간 발사. 데이터 확장판(견본행 합성)은 다음 세션.

### 🎯 상시 지시(2026-08-13, 사용자): 로컬 소형모델의 표 채움·보고서 작성을 Opus 수준으로 — 지속 개선 루프
반복 사이클: **격차 실측(비전 검수 필수) → 원인 분해 → 하네스 우선(지능을 코드로) → 남는 것만 학습(증류/RL) → 벤치+실물 PDF 검증 → 텔레그램 보고**. 현재 루프 상태:
- 진행 중: v14 학습(v13셋 2ep, 야간). 완료 시 미학습 벤치 + doc9 견본행 표 재검증.
- 다음 카드(우선순위): ①**견본행 교체 예제 합성**(1~5행 정렬 혼선을 직접 가르치는 v15 데이터 — make_fill_dataset 확장: 표 상단에 견본값 주입 후 실값 교체를 gold로) ②**보고서(표 밖 본문) 작성 기능** — 서식3류 "가. 목적"/"1) 교육 횟수:" 본문 라인은 빈칸 목록에 없어 전 모델 미채움: 콜론말미 본문 라인을 body 슬롯으로 추출 + 공문 어댑터로 문장 생성 ③자유문장 선채움(의미 매칭 LLM 2패스: 추출→배치) ④GRPO(자동 채점 보상) ⑤확신도 승인 패널 연동.
- 판정 기준: 미학습 양식 실물 PDF에서 Opus 산출과 나란히 놓고 차이 셀 수를 세는 것(비전). 현재 격차: 표 채움 문서당 ~2-5셀(견본행 표), 보고서 작성 미지원.

### §42m v14 채택 — 자유문장 96% 신기록 (2026-08-14)
- **v14**(v13셋 2ep, loss 0.020, 21.5h 주간 학습): 정형 93%(v12 동급권) · **자유문장 96%**(v12 92% → +4%p, Opus -4%p) → **채택**(실전 지표 우선). `fill_g4e4b_v14.gguf`.
...
### §42n v14 제품 배선 + 어댑터 가시화 (2026-08-14 오후)
- **핸드오프 "미통합" 해소(Desktop 개발 프로필)**: 3개 구멍을 막음. `05a0794`·`91bcc69`.
  1. `data/settings.json`: local_model=**gemma-4-E4B-it-Q4_K_M**(정확 파일명 — 부분일치가 같은 폴더 F16 14GB를 집는 것 방지, §41b 등급 일치) · local_lora=gongmun_g4e4b_v10(id0) · **fill_lora=fill_g4e4b_v14(id1)** · llama_server_bin=b10338 vulkan. 이전 프로필은 `settings.json.bak_e2b_v9` 보존. **배포 번들 무영향**(번들은 stage_bundle.py DEFAULT_SETTINGS, frozen은 %LOCALAPPDATA%).
  2. `engine/llm_manager.py`: reasoning=off 시 `--reasoning-budget 0` 동반 + 페이로드 `chat_template_kwargs:{"enable_thinking":false}`(§41f 함정 — b10298은 무시라 무해) + **바이너리 후보 순서 재정렬**(b10338·vulkan을 구 CUDA `llama_cpp/bin`보다 앞에 — 기존 순서로는 개발기에서 구빌드가 조용히 선택되고 있었음, 로그 실측).
  3. UI 가시화: `/api/loras`에 `role`("생성"/"채움")·`fill_active` 추가, FormAssist 모델 드롭다운 아래 + PropertiesPanel LoraSelector에 "채움 어댑터 fill_g4e4b_v14 자동 적용" 표시. **채움 어댑터는 사용자가 고르는 값이 아니라 채움 요청에서 코드가 켜는 id1**임을 UI가 말하게 함(기존엔 적용 여부를 확인할 길이 없었음).
- **검증**: 기동 커맨드 = b10338 + `--reasoning off --reasoning-budget 0 --jinja` + 2어댑터 프리로드(`/lora-adapters` id0/id1 확인) · health nodes=31 · 제품 E2E(체험학습양식, provider=local, 12.6s) **4/5 정확**.
- **신규 결함 실측(오배치 1건)**: 지시문의 "대상=2학년 전체 120명"이 양식에 없는 라벨이라 **남는 빈칸(준비물)에 밀려 들어감** — 잉여값 기각 미학습. v15 부정 예제 카드로 승격(fill_retry 상호작용 가설은 미검증).
- **사용자 라이브 테스트 관찰**: 빈 한글 문서에 "아침·점심·저녁 표 만들고 채워라"(채팅 라이브 경로) → 품질 불량. **이 경로는 fill_v14 무관**(채움 어댑터는 form_assist 채움 요청에만 켜짐) — 라이브 표 생성·편집 액션은 베이스+GBNF뿐, 학습 0. "편집 액션 학습쌍 = 유일 미검증 축"(§16·§41h 이후 반복 지적)이 실사용자 체감으로 확인된 첫 사례. → **§43에서 하네스로 해소.**

### §43 라이브 표 생성 하네스 — "표 만들고 채워라" 뒤죽박죽 해소 (2026-08-14 오후)
- **원인 실측(E2E 재현)**: E4B가 `create_table(4×4, data=헤더 1행만)`을 낸 뒤 나머지를 **지어낸 block_id 1~4**에 `replace_cell_content`로 채움 — 새 표의 블록 번호는 스캔 목록에 없으므로 반드시 허구. 게다가 행 전체를 한 셀에 `\n`으로 뭉침. 실행하면 엉뚱한 블록을 덮어 표가 뒤죽박죽 = 사용자가 본 증상 그대로. **스킬 지시("data 한 방에")를 추가해도 소형모델은 여전히 쪼갬을 실측** — 프롬프트 방어 불충분, 하네스 몫.
- **수정 2겹(지능을 코드로)**:
  1. **`normalize_table_spec`** (`engine/hwp/editor.py`): create_table의 셀 채움이 탭 워킹이라 rows/cols·data 치수가 어긋나면 셀이 통째로 밀림 — GBNF가 강제 못 하는 부분(params 자유 객체). data가 있으면 치수의 진실원천은 data: 균일 2D 강제(1D→한 행, records→헤더+값, ragged→패딩), 선언이 더 크면 빈 행/열 의도 존중(max), 상한 100×30. 패딩된 균일 data는 탭 스텝 수가 표와 정확히 일치 → 밀림 원천 차단.
  2. **`merge_new_table_fills`** (`engine/chat/live_chat.py`): 같은 배치에서 create_table 뒤의 채움류(replace_cell_content/replace_table_row/append_table_row) 중 block_id가 **현재 스캔의 실존 표 셀(td)이 아닌 것**을 행 데이터로 해석(row_texts 그대로, new_text는 `\n` 분할)해 create_table.data에 병합, 배치에서 제거. 실존 셀 편집은 불변. 비스트림(`handle_live_chat`)·스트림(`/api/chat/live/stream`) 양쪽 배선 — **승인 패널에도 병합안이 보임**.
- **검증**: 오프라인 **149 PASS 0 FAIL**(신규 19: 정규화 9 + 가짜COM 탭워킹 착지 3 + 병합 7) + **실물 E2E**(빈 한/글 + local E4B, 사용자 시나리오 그대로): 병합 발동 → "5×4 표 생성 완료, 20셀 채움" → CVD 재스캔 = 헤더(요일·아침·점심·저녁)+4요일 행 전셀 정위치, 밀림·뭉침 0. 헤더 서식·열 너비도 정상.
- **남은 것**: 이번 건은 하네스로 종결이나, 라이브 편집 축의 **block_id 오선택**(기존 문서 편집 시) 일반 문제는 여전히 미학습 — 편집 액션 학습쌍 카드는 유효.

### §43b 라이브 연쇄 사용 시 "오류" 고정 해소 — 컨텍스트 초과 3겹 방어 + clear_document (2026-08-14 오후)
- **사용자 실사용 3턴째 "오류" 연발** → llama-server 로그로 원인 확정: `request (15723 tokens) exceeds the available context size (8192)`. 구성: 문서 커짐(표 2개, CVD 3.5k자) + **UI가 실행 결과 로그(✓줄 ×36)를 assistant 메시지에 누적하고 그대로 히스토리 재전송** + 스킬 프롬프트. 한 번 초과하면 이후 모든 요청이 실패로 고정.
- **방어 3겹**: ①ctx 8192→**16384**(settings, 16GB GPU 실측 4.9GB 사용 — 여유) ②`trim_live_history`(live_chat, 양 경로): 결과 로그 줄(✓✗⏳) 제거 + 메시지당 1,200자 캡 + 최근 10개 ③llm_manager 컨텍스트 초과 시 **히스토리 축소 자동 재시도**(system=스킬+문서 보존, 최근 4개→1개 순 — 실패 고정 대신 오래된 대화를 버리고 살림).
- **`clear_document` 액션 신설**(24종째): "다 지우고 다시" 요청이 실행 불가능했음(카탈로그에 전체 삭제가 없어 모델이 덧붙이기만 함). editor(전체선택→삭제+블록맵 클리어) → controller → live 매핑 → GBNF enum → 스킬 문서까지 배선.
- **부수 함정(재발 방지)**: 프론트 재기동 시 `ENGINE_PORT=8407` 누락하면 vite 프록시가 기본 8406을 봐서 **"한/글 연결 실패 (서버 오류)"** — 엔진에 요청이 아예 안 닿는 증상. PowerShell 호출 간 env 비유지 주의.
- **검증**: 오프라인 **156 PASS 0 FAIL**(신규 7) · 실기동 n_ctx_slot=16384 확인 · 표 2개 문서에서 로컬 E4B 질의 정상 응답(액션 0, 문서 내용 정확 요약).
- **남은 관찰 1건**: 2턴째에 모델이 새 12×5 표를 만들면서 **기존 표의 실존 셀들도 수정**(정당 편집으로 분류돼 병합 대상 아님) — 결과는 성공이나 의도(새 표만)와 어긋났을 수 있음. "수정 대상 표 명시" 프롬프트 규칙 또는 편집 액션 학습쌍 몫.

### §43c 프롬프트 해부 + 구형 노트북 판정 (2026-08-14 저녁 — 분석만, 구현은 다음 세션)
- **"몇 턴 안 했는데 왜 8192 초과?" 해부**: 대화가 아니라 **문서 동봉** 구조. 15.7k 내역 ≈ 스킬 2.5k + **문서 원문 3k + 같은 내용의 CVD 블록맵 3.5k(2중 동봉!)** + 히스토리(결과로그 포함) 2k + max_tokens 예약 4k. `_read_with_cvd`가 init_scan 원문을 붙인 뒤 CVD를 또 붙이는데 **CVD에 전체 텍스트가 이미 있다** — 프롬프트 비만의 주범. 문서가 커질수록 다음 턴 프롬프트가 커지는 구조라 잘 쓸수록 빨리 터짐.
- **⏭️ 다음 세션 확정(사용자): 프롬프트 다이어트 → 구형 실측** 순서.
  1. **원문/CVD 2중 동봉 제거** — 라이브 프롬프트 반토막(용지 정보는 보존, 원문 스캔부를 CVD로 일원화). 벤치·라이브 E2E로 품질 회귀 확인 필수(모델이 원문 서술형에 기대던 부분 있는지).
  2. **KV 프리픽스 캐시 재배치** — 고정부(스킬 문법책)를 앞, 가변부(문서 상태)를 뒤로 → llama.cpp cache_prompt 재사용으로 구형 프리필 체감 직결. 현재는 {document_content}가 스킬 상단이라 매 턴 캐시 무효.
  3. **Notebook2(학교 상주, 교사컴 근사) E2B 실측** — 축별 체감표(표채움/공문/라이브 각 턴당 초). 다이어트 후 재는 것이 순서.
- **구형 노트북 가부 판정(§9·§19 실측 종합)**: 표채움·추출·공문 = **E2B로 검증 완료, 30초~2분**(선채움은 LLM 0회라 하드웨어 무관). 장문 = API 권장(§9 확정). **라이브 채팅 편집 = 가장 약한 축**(E2B 자유편집 열화 + 매 턴 프리필 수천 토큰 — 구형 CPU에서 턴당 1~3분 추정). §43~43b 하네스는 코드라 하드웨어 비용 0 — 약한 모델일수록 더 벌어줌. 배포 번들 ctx는 memory_manager 자동(16384는 Desktop 개발 프로필 한정) — ctx 작은 구형에서 §43b 자동회복이 더 자주 구원.

### §43d 라이브 프롬프트 다이어트 — 턴당 처리 토큰 -57% 실측 (2026-08-16)
- **구현 2겹**:
  1. **원문/CVD 2중 동봉 제거**(`_read_with_cvd`): CVD에 전체 텍스트가 블록별로 이미 있으므로 init_scan 원문은 프롬프트에서 제외, 용지 정보(CVD에 없는 레이아웃)만 유지. CVD 실패 시엔 기존 원문 폴백 유지.
  2. **고정부 앞·문서 뒤 조립**(`assemble_live_prompt` 신설): 스킬 본문→디자인→응답형식(전부 고정) 뒤에 `## 현재 문서 상태`(가변)를 맨 끝에. {document_content}가 상단이면 문서가 바뀔 때마다 llama.cpp 프리픽스 캐시 전체 무효였음. 템플릿 4종(hwp/excel/ppt/word)에서 상단 문서 섹션 제거, 구 플레이스홀더는 하위호환(위치 안내로 대체). 비스트림·스트림 양 경로 공용.
- **A/B 실측(같은 시나리오: 빈 문서→식단표 생성→질문, E4B Q4)**: 턴1 2,764→2,787(동일 — 빈 문서라 효과 없음이 정상) · **턴2 3,779→1,624 (-57%)** — 중복 제거 + 턴1 고정 프리픽스 캐시 재사용의 합. 응답 품질 동일(질문 정답). 문서가 클수록 절감 커짐(중복분이 문서 크기에 비례).
- **구형 환산**: 구형 CPU 프리필 ~50-100 t/s 가정 시 턴당 38-76s → **16-32s**. 라이브 축 실용성 유의미 개선.
- **검증**: 오프라인 **160 PASS 0 FAIL**(신규 4 — 문서 맨뒤·프리픽스 동일성(캐시 보장)·구 플레이스홀더 호환·템플릿 4종 제거 확인).
- **함정(재발 방지)**: 한/글 강제종료 후 COM 연결이 -2146959355(서버 실행 실패)로 죽는 경우 — Hwp.exe를 먼저 수동 기동하면 붙는다(`C:\Program Files (x86)\Hnc\Office 2020\HOffice110\Bin\Hwp.exe`).
- **다음**: Notebook 실측(E2B 축별 체감표 — Notebook2는 16일째 오프라인이라 Notebook(100.89.219.102) 대체 후보), 그 다음 v15 데이터.

### §43e 구형 노트북 CPU 실측 — Desktop 시뮬레이션 (2026-08-16, 타 장비 전부 오프라인)
- **순수 CPU**(`llama-bench -dev none` — ⚠ `-ngl 0`만으론 Vulkan이 프리필을 가속해 무의미, dev none 필수)로 E2B Q4_K_M 실측(9800X3D): pp2048 **260 t/s**(4T)/**403 t/s**(8T) · tg128 **31.5 t/s**.
- **환산(§43d 다이어트 반영, 라이브 턴 = 프리필 ~1,624tok + 생성 ~150tok)**: 최신 CPU 4T ≈ **11초/턴** → 구형 노트북(성능 1/3~1/5) ≈ **25~60초/턴**(다이어트 전 40~100초). 표채움 청크(~800tok)는 더 짧음. 실물 구형 실측은 워커1호/Notebook2 부활 시.

### §42o v15 데이터·학습 발사 — 견본행 교체 + 잉여값 부정예제 (2026-08-16)
- **v15 = A(v13 기본 2,124 정확 재현) + B(견본행 136) + C(잉여값 부정 389) = 2,649** (`scripts/lora/make_fill_dataset_v15.py`, 다양화 519 승계).
  - **B 견본행**: 표의 첫 gold 행 셀에 ○류 스캐폴드 주입(김○○·○○초·○월 ○일(○) — 전부 런타임 `_is_placeholder` 통과를 속성검사로 보장) → 렌더에 견본 텍스트로 보임 + 빈칸 목록에 라벨 동봉(런타임 형식) → gold = 실값 교체. v14 잔여 결함(상단 행 순번→날짜 복사)의 직접 처방.
  - **C 잉여값**: 지시문에 가짜 라벨:값 추가 + 지시문이 못 채우는 실제 빈칸을 목록에 추가 → gold 불변(가짜값 미사용·모르는 빈칸 미채움). §42n 준비물 오배치의 직접 처방.
  - 검증 PASS: B 속성검사 713건 0실패 · C gold 오염 0.
- **제품 버그 발견·수정(데이터 검증의 부수 수확)**: `_is_placeholder` 상단 `len>12` 조기 반환이 §42l 혼합견본 30자 분기를 **13~30자 구간 데드코드**로 만들고 있었음("김○○13.03.11(남)" 15자 실존 견본 미감지) → 12자 규칙과 30자 혼합 규칙 분리. 회귀 **163 PASS**(케이스 3개 박제).
- **학습 발사**: r16@1536 2ep(v14 레시피), 242스텝. 1536 생존 = A 76%/B 69%(95건)/C 75% → train 1,922/val 78. D: 12GB 확보(구 어댑터 원본 5개 3.7GB → `E:\sbc_lab\_archives\lora_out` 이동, GGUF는 보존).
- **무인 체인 가동**: `E:\sbc_lab\tf_build\v10_chain\v15_chain.py` — 완료 감지 → GGUF 변환 → 벤치(정형 unseen + 자유문장) → **텔레그램 보고** + 서버 정리. 판정 = v14(정형 93/자유 96) 대비, doc9류 견본행 실물 재검(비전)은 다음 세션.

### §42p v15 결과 — 채택 보류, v14 유지 (2026-08-17)
- **학습**: 2ep 완주(18.1h, train_loss 0.020 — v14 동급), `fill_g4e4b_v15.gguf`(133MB) 변환 OK.
- **벤치(미학습 30양식/364필드, 고정 프로토콜)**:
  | | v14 | **v15** | Δ |
  |---|---|---|---|
  | 정형 선채움+어댑터 | 93% | **95%** | +2%p |
  | 정형 어댑터 단독 | — | 92% | |
  | 자유문장 어댑터 단독 | **96%** | **91%** | **-5%p** |
  | 자유문장 +3표결 | — | 92% | |
- **판정: 채택 보류(v14 유지)** — 정형 +2%p 대가로 실전 지표(자유문장) -5%p 후퇴. §42m 채택 근거(자유문장 96)를 깎으면 불가.
- **원인 가설(미검증)**: C 잉여값 예제 389건의 지시문 꼬리("그리고 X은(는) Y입니다")가 **정형 템플릿 말투를 대량 추가** → 자유문장 일반화 희석. 정형↑·자유↓ 방향과 정확히 일치. 대안 v16 = C 감량(1/3) 또는 C 지시문을 Sonnet 다양화(§42b 파이프 재사용).
- **본 시험 미실시**: v15의 목적인 **견본행 표(doc9류) 실물 비전 재검**은 아직. 여기서 v14가 못 하던 상단행이 잡히면 v16 데이터 조정만으로 채택 가능 — 다음 카드 1순위.
- **인프라 결함 2건(실측)**: ①체인이 학습 venv(`D:\lora_train\venv`)로 벤치를 돌려 **lxml 부재 → 두 벤치 빈손**(fill_chain.py의 PY 변수를 그대로 베낀 실수) → 벤치는 시스템 py(`BENCH_PY`)로 분리 수정 ②수정 후에도 빈손 → 어댑터 로드로 서버 기동이 60초 대기를 넘긴 것으로 추정(직접 재현은 정상) — **체인 헬스 대기 연장 + 빈 출력 = 실패 처리** 필요. ③02:54 TG 발송이 DNS 실패(getaddrinfo) — 새벽 네트워크 일시 단절, 결과는 로그(`v15_chain.log`)에 남음. 벤치 실측은 `scratchpad/bench_capture_debug.py`(체인 캡처 방식 재현)로 수동 완료.

### §44 Qwen3.8-27B(8/14 출시) Desktop 구동 검증 (2026-08-17 — 별건, TeacherFlow 배포 무관·판정자/연구용)
- **자산**: `E:\llm\models\Qwen3.8-27B-Q4_K_M.gguf`(15.93GiB, unsloth) + **llama.cpp b10453**(`E:\sbc_lab\llama_test\b10453`, 신아키텍처 지원 — b10338은 8/14 모델 미지원 가정). IQ4_XS(15.7GB) 추가 수신 중.
- **스모크 전 항목 통과**: 로드·헬스 12s · 추론 정답 · `--jinja` 턴경계 정상 · 사고 토글(`chat_template_kwargs:{"enable_thinking":false}` → 직답 / 기본 → reasoning_content 분리) — **§41f 프로토콜 그대로 통함**.
- **프로필**: `-ngl 56 -np 1 -fa on -c 8192 --jinja` → VRAM 13.7GB, **생성 5.2~5.7 t/s**(16GB에 17.1GB라 일부 레이어 RAM — CPU 구간이 병목). 판정·배치용(건당 ~10s) 실용, 대화형 부적합.
- **모델 특성**: 하이브리드 어텐션(64층 중 16층만 KV) → KV 토큰당 64KB(일반 27B의 1/4). 16GB에 전량 탑재 가능성이 있는 유일한 27B.
- **IQ4_XS 전량 GPU 실측**(`E:\llm\models\Qwen3.8-27B-IQ4_XS.gguf` 14.63GiB, `-ngl 99 -np 1 -fa on`): **VRAM 14.8GB에 전량 탑재 성공**(여유 1.2GB). 서버 자체 timings = **생성 8.8 t/s · 프리필 67 t/s**. Q4_K_M 부분오프로드(5.2)보다 1.6배 빠르지만 기대(15~20)엔 못 미침 — 특히 프리필 67은 5080 치곤 비정상적으로 낮음. 원인 후보(미검증): IQ 계열의 Vulkan 커널 비효율(알려진 약점) / b10453 신아키텍처 커널 미최적화. **CUDA 빌드(`llama-b10453-bin-win-cuda-12.4-x64.zip`)로 재측정이 다음 카드** — 5080은 CUDA가 정공법이고, 이 프로젝트가 vulkan을 쓰는 이유는 배포 대상(iGPU 노트북)이지 개발기 성능이 아님.
- **판정 프로필(현시점)**: 판정·배치 = IQ4_XS ngl99(8.8 t/s, 건당 60tok ≈ 7s → 378건 ≈ 45분). 대화형 부적합. 워커 8085 등록은 CUDA 재측정 후.
- **함정**: 재부팅 시 워커(SBC-LLM-Agent)가 9B를 8084에 자동 기동(VRAM 7.9GB) — 27B 전에 `POST :9090/worker {"action":"stop"}` 필수. `-ngl 0`만으론 Vulkan이 프리필 가속(순수 CPU 측정은 `-dev none`). Start-Process 리다이렉트로는 llama-server 로그가 안 남음(0바이트) — 타이밍은 응답 `timings` 필드 또는 `/slots`로.
...
**대기 중 (사용자 승인 후 착수)**:
1. **제품 통합·배포 마찰 제거** (§39 정찰 결론 = 우리 최대 약점):
   - **코드 서명** — 현재 무서명이라 V3가 차단(§28 실측). 배포의 1순위 병목.
   - **단일 관문화** — 교사용 입구를 TeacherFlow 하나로. 별도 앱(022·026·008·019 등) 기능은 노드/프리셋으로 흡수. "앱 개수는 자산이 아니라 부채".
   - **실사용자 1명 확보** → §36 피드백 로그가 실데이터로 전환되는 시점.
2. **큰 문서 처리** (사용자 제안, 8/10):
   - 서술형 초과분: 오버랩 슬라이스 + **스키마 강제 사실 추출**(요약 금지 — 소형모델 임의 탈락 방지) → 병합 → 2단 파이프 개요 주입.
   - 초대형: 코드가 목차 생성 → **코드가 관련 섹션 선별** 투입(모델이 여는 다회전 루프는 E2B에 부담). `kordoc --format chunks`(표 독립 청크+breadcrumb) 흡수.
3. **SchoolBoard 채택 후보**(§39): confirmedLabel 교차검증 → fill_column(명단형) → 적용 전 로컬 2차 감사 → expectedCurrent 덮어쓰기 가드.
4. **v10 후보**: 계획서 혼선 해소(v8·v9 공통 5/5 결함 — 계획서 업웨이트 강화) · **편집 액션 학습쌍 합성**(block_id 오선택 = 유일 미검증 축) · E4B판 A/B 후 "고품질 팩" 결정.
5. 잔여 소품: 견본혼합형(○○과) 감지 · xlsx 채움 하드벤치 · 온보딩/모델팩 UI · `kordoc patch` vs 우리 라운드트립 비교 · kordoc_watch 주기 등록.
