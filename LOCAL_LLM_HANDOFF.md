...
- [ ] FormAssist가 로컬 LoRA를 provider 선택지에서 제외(`FormAssist.tsx:47`)
- [ ] 내 워크플로우를 홈카드로 등록하는 UI 부재(백엔드는 존재)
...
- `6b8392a` 채팅 기본모델 = active 모델(429 수정) · `2157d9b` 실행 피드백 4종(블로킹/에러/스피너/출력파일) · `621b757` 워크플로우 생성 검증 + auto_layout 버그(source/target→from/to) · `ddef444` 공문 초안 작성기 프리셋 + TaskRunner 라벨 메타 · `7204e62` 설계캔버스 3종(Ctrl+S/미저장/파라미터)
- **검증**: offline 테스트 PASS · validate/auto_layout 단위테스트 · 프리셋 E2E(hwpx 출력) · 브라우저 홈카드·입력화면 확인 · tsc 클린
- **다음 세션 이어서**: H/S 잔여 2건(FormAssist 로컬 LoRA 노출, 워크플로우→홈카드 등록 UI) → H/M(취소·GBNF워크플로우생성·Ctrl+Z·연결피드백·카드실행·고빈도프리셋) → M/S(자잘). 전문 `D:\lora_data\tf_improvement_findings.json`.
