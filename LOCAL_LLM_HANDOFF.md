...
### 데이터 소스 (사용자 확정)
- **`G:\내 드라이브\0.신병철의 Drive\0000. 2024학년도\2.교무부 업무\0. 인수인계`** (구글 드라이브 스트림)
...
- **로컬 복사**: `D:\lora_data\raw\{기안문,접수문}` (odt/hwp/hwpx만 robocopy, G:는 느리고 불안정). ⚠️ 가정통신문 유형은 이 소스에 없음 — 사용자에게 추가 폴더 요청해둠(v1은 공문체만).
