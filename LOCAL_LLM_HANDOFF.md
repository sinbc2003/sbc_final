...
**H/S (고효과·저비용 — 최우선)**
- [✅] 채팅 기본모델이 OpenAI 하드코딩 → 로컬 설정 무시하고 **429 유발**(사용자 실사용 보고). ChatMode 모델목록에서 `active` 선택(`6b8392a`).
- [ ] TaskRunner가 엔진의 친절한 한국어 에러를 버리고 범용 문구만(`TaskRunner.tsx:213`)
- [ ] 실패 노드가 캔버스에서 영원히 '실행 중' 스피너(`store.ts:306`)
- [ ] run-stream done 이벤트에 output_files 없어 프론트가 정규식 추측(`execution.py:138`)
- [ ] `/api/run`이 이벤트 루프 블로킹 — 실행 중 서버 전체 정지(`execution.py:52`, run_in_executor)
- [ ] 생성 워크플로우 노드type/엣지/포트 저장 전 코드 검증 부재(`chat/workflow.py:113`)
- [ ] Ctrl+S stale closure → 매번 새 워크플로우 중복 생성(`Toolbar.tsx:42`)
- [ ] 미저장 무경고 유실(새로만들기/불러오기/탭닫기, `Toolbar.tsx:93`+beforeunload)
- [ ] 파라미터 description UI 미표시 + 숫자필드 NaN→null(`PropertiesPanel.tsx:160`)
- [ ] **공문 LoRA 원클릭 프리셋 홈 노출 부재**(`HomeScreen.tsx:101`) — 방금 만든 핵심기능 미노출
...
### 구현 기록
- `6b8392a` (H/S) 429 원인 = 채팅 기본모델 하드코딩 → active 모델 선택으로 수정.
